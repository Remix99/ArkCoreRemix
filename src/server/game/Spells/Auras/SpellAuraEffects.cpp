...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS] = { &AuraEffect::HandleNULL, //  0 SPELL_AURA_NONE
        ...
        &AuraEffect::HandleNoImmediateEffect, //314 SPELL_AURA_PREVENT_RESURRECTION todo
...
AuraEffect::AuraEffect(Aura *base, uint8 effIndex, int32 *baseAmount,
        Unit *caster) :
        m_base(base), m_spellProto(base->GetSpellProto()), m_effIndex(effIndex), m_baseAmount(
                baseAmount ?
                        *baseAmount :
                        m_spellProto->EffectBasePoints[m_effIndex]), m_canBeRecalculated(
                true), m_spellmod(NULL), m_isPeriodic(false), m_periodicTimer(
                0), m_tickNumber(0) {
...
void AuraEffect::GetTargetList(std::list<Unit *> & targetList) const {
    ...
    for (Aura::ApplicationMap::const_iterator appIter = targetMap.begin();
            appIter != targetMap.end(); ++appIter) {
        if (appIter->second->HasEffect(GetEffIndex()))
            targetList.push_back(appIter->second->GetTarget());
    }
}

void AuraEffect::GetApplicationList(
        std::list<AuraApplication *> & applicationList) const {
    Aura::ApplicationMap const & targetMap = GetBase()->GetApplicationMap();
    // remove all targets which were not added to new list - they no longer deserve area aura
    for (Aura::ApplicationMap::const_iterator appIter = targetMap.begin();
            appIter != targetMap.end(); ++appIter) {
...
int32 AuraEffect::CalculateAmount(Unit *caster) {
    ...
    amount = SpellMgr::CalculateSpellEffectAmount(m_spellProto, m_effIndex,
            caster, &m_baseAmount, NULL);
                    ...
                    if (castItem->GetItemSuffixFactor()) {
                        ItemRandomSuffixEntry const *item_rand_suffix =
                                sItemRandomSuffixStore.LookupEntry(
                                        abs(
                                                castItem->GetItemRandomPropertyId()));
                        if (item_rand_suffix) {
                            for (int k = 0; k < MAX_SPELL_EFFECTS; k++) {
                                ...
                                if (pEnchant) {
                                    for (int t = 0; t < MAX_SPELL_EFFECTS; t++)
                                        if (pEnchant->spellid[t]
                                                == m_spellProto->Id) {
                                            amount =
                                                    uint32(
                                                            (item_rand_suffix->prefix[k]
    ...
    case SPELL_AURA_TRANSFORM:
        ...
        if (caster) {
            ...
            Unit::AuraEffectList const& overrideClassScripts =
                    caster->GetAuraEffectsByType(
                            SPELL_AURA_OVERRIDE_CLASS_SCRIPTS);
            for (Unit::AuraEffectList::const_iterator itr =
                    overrideClassScripts.begin();
                    itr != overrideClassScripts.end(); ++itr) {
    ...
    case SPELL_AURA_SCHOOL_ABSORB:
        ...
        case SPELLFAMILY_MAGE:
            ...
            // Frost Ward
            else if (GetSpellProto()->SpellFamilyFlags[0] & 0x100
                    && GetSpellProto()->SpellFamilyFlags[2] & 0x8) {
                // +80.68% from sp bonus
                DoneActualBenefit += caster->SpellBaseDamageBonus(
                        GetSpellSchoolMask(m_spellProto)) * 0.8068f;
    ...
    case SPELL_AURA_MANA_SHIELD:
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_MAGE
                && GetSpellProto()->SpellFamilyFlags[0] & 0x8000
                && m_spellProto->SpellFamilyFlags[2] & 0x8) {
            // +80.53% from +spd bonus
            DoneActualBenefit += caster->SpellBaseDamageBonus(
                    GetSpellSchoolMask(m_spellProto)) * 0.8053f;
            ;
        }
        break;
    case SPELL_AURA_DUMMY:
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_SHAMAN
                && m_spellProto->SpellFamilyFlags[1] & 0x400) {
            // return to unmodified by spellmods value
            amount = m_spellProto->EffectBasePoints[m_effIndex];
            // apply spell healing bonus
            amount = caster->SpellHealingBonus(GetBase()->GetUnitOwner(),
                    GetSpellProto(), GetEffIndex(), amount,
                    SPELL_DIRECT_DAMAGE);
            // apply spellmods
            amount = caster->ApplyEffectModifiers(GetSpellProto(), m_effIndex,
                    float(amount));
        }
        break;
    case SPELL_AURA_DAMAGE_SHIELD:
        ...
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID
                && m_spellProto->SpellFamilyFlags[0] & 0x100)
            // 3.3% from sp bonus
            DoneActualBenefit = caster->SpellBaseDamageBonus(
                    GetSpellSchoolMask(m_spellProto)) * 0.033f;
        break;
    case SPELL_AURA_PERIODIC_DAMAGE:
        ...
        // Rend
            ...
            float mwb_min = caster->GetWeaponDamageRange(BASE_ATTACK,
                    MINDAMAGE);
            float mwb_max = caster->GetWeaponDamageRange(BASE_ATTACK,
                    MAXDAMAGE);
            amount += caster->ApplyEffectModifiers(m_spellProto, m_effIndex,
                    int32(((mwb_min + mwb_max) / 2 + ap * mws / 14000) * 0.2f));
            // "If used while your target is above 75% health, Rend does 35% more damage."
            // as for 3.1.3 only ranks above 9 (wrong tooltip?)
            if (sSpellMgr->GetSpellRank(m_spellProto->Id) >= 9)
                if (GetBase()->GetUnitOwner()->HasAuraState(
                        AURA_STATE_HEALTH_ABOVE_75_PERCENT, m_spellProto,
                        caster))
                    amount += int32(
                            amount
                                    * SpellMgr::CalculateSpellEffectAmount(
                                            m_spellProto, 2, caster) / 100.0f);
        }
    ...
    case SPELL_AURA_PERIODIC_ENERGIZE:
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_GENERIC) {
            ...
            if (m_spellProto->SpellIconID == 3184
                    && m_spellProto->SpellVisual[0] == 12495)
                amount = GetBase()->GetUnitOwner()->GetMaxPower(POWER_MANA) * 25
                        / 10000;
        }
        // Innervate
    ...
    case SPELL_AURA_PERIODIC_HEAL:
        if (!caster)
            break;
        // Lightwell Renew
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_PRIEST
                && m_spellProto->SpellFamilyFlags[2] & 0x4000) {
            if (caster->GetTypeId() == TYPEID_PLAYER)
                // Bonus from Glyph of Lightwell
                if (AuraEffect* modHealing = caster->GetAuraEffect(55673, 0))
                    amount = int32(
                            amount * (100.0f + modHealing->GetAmount())
                                    / 100.0f);
    ...
    case SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN:
        if (!caster)
            break;
        // Icebound Fortitude
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT
                && m_spellProto->SpellFamilyFlags[0] & 0x00100000) {
            if (caster->GetTypeId() == TYPEID_PLAYER) {
                int32 value = -amount;
                ...
                uint32 defval = uint32(
                        caster->ToPlayer()->GetSkillValue(SKILL_DEFENSE)
                                + caster->ToPlayer()->GetRatingBonusValue(
                                        CR_DEFENSE_SKILL));
                if (defval > 400)
        ...
        // Hand of Salvation
        else if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_PALADIN
                && GetSpellProto()->SpellFamilyFlags[0] & 0x00000100) {
            //Glyph of Salvation
    ...
    case SPELL_AURA_MOD_INCREASE_ENERGY:
        // Hymn of Hope
        if (GetId() == 64904)
            amount = GetBase()->GetUnitOwner()->GetMaxPower(
                    GetBase()->GetUnitOwner()->getPowerType()) * amount / 100;
        break;
    case SPELL_AURA_MOD_INCREASE_SPEED:
        // Dash - do not set speed if not in cat form
        if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID
                && GetSpellProto()->SpellFamilyFlags[2] & 0x00000008)
            amount =
                    GetBase()->GetUnitOwner()->GetShapeshiftForm() == FORM_CAT ?
                            amount : 0;
        break;
    case SPELL_AURA_MOUNTED: {
        ...
        if (plr) {
            ...
            const MountTypeEntry *type = sMountTypeStore.LookupEntry(
                    GetMiscValueB());
            if (!type)
                return 0;
            ...
            uint32 map = GetVirtualMapForMapAndZone(plr->GetMapId(),
                    plr->GetZoneId());
            uint32 maxSkill = 0;
            for (int i = 0; i < MAX_MOUNT_TYPE_COLUMN; i++) {
                const MountCapabilityEntry *cap =
    ...
    GetBase()->CallScriptEffectCalcAmountHandlers(
            const_cast<AuraEffect const *>(this), amount, m_canBeRecalculated);
...
void AuraEffect::CalculatePeriodic(Unit *caster, bool create) {
    ...
    GetBase()->CallScriptEffectCalcPeriodicHandlers(
            const_cast<AuraEffect const *>(this), m_isPeriodic, m_amplitude);
...
void AuraEffect::CalculateSpellMod() {
    ...
    case SPELL_AURA_ADD_PCT_MODIFIER:
        if (!m_spellmod) {
            ...
            m_spellmod->spellId = GetId();
            m_spellmod->mask =
                    GetSpellProto()->EffectSpellClassMask[GetEffIndex()];
    ...
    GetBase()->CallScriptEffectCalcSpellModHandlers(
            const_cast<AuraEffect const *>(this), m_spellmod);
}
...
void AuraEffect::ChangeAmount(int32 newAmount, bool mark) {
    ...
    if (newAmount != GetAmount()) {
        ...
        for (UnitList::iterator aurEffTarget = targetList.begin();
                aurEffTarget != targetList.end(); ++aurEffTarget) {
            HandleEffect(*aurEffTarget, AURA_EFFECT_HANDLE_CHANGE_AMOUNT,
                    false);
        }
        if (!mark)
            m_amount = newAmount;
...
void AuraEffect::HandleEffect(AuraApplication const *aurApp, uint8 mode,
        bool apply) {
    ...
    if (apply)
        prevented = GetBase()->CallScriptEffectApplyHandlers(
                const_cast<AuraEffect const *>(this), aurApp,
                (AuraEffectHandleModes) mode);
    else
        prevented = GetBase()->CallScriptEffectRemoveHandlers(
                const_cast<AuraEffect const *>(this), aurApp,
                (AuraEffectHandleModes) mode);
...
void AuraEffect::HandleEffect(Unit *target, uint8 mode, bool apply) {
    AuraApplication const *aurApp = GetBase()->GetApplicationOfTarget(
            target->GetGUID());
    ASSERT(aurApp);
...
void AuraEffect::ApplySpellMod(Unit *target, bool apply) {
    ...
    case SPELLMOD_EFFECT3: {
        ...
        for (Unit::AuraApplicationMap::iterator iter = auras.begin();
            ...
            if ((aura->IsPassive()
                    || aura->GetSpellProto()->AttributesEx2
                            & SPELL_ATTR2_ALWAYS_APPLY_MODIFIERS)
                    && aura->GetCasterGUID() == guid
                    && sSpellMgr->IsAffectedByMod(aura->GetSpellProto(),
                            m_spellmod)) {
            ...
            case 92803:
                ...
                if (AuraEffect * aurEff = GetBase()->GetEffect(0)) {
                    if (aurEff->GetAuraType() != SPELL_AURA_MOD_POWER_REGEN) {
                        m_isPeriodic = false;
                        sLog->outError(
                                "Aura %d structure has been changed - first aura is no longer SPELL_AURA_MOD_POWER_REGEN",
                                GetId());
                    } else {
...
void AuraEffect::PeriodicTick(AuraApplication * aurApp, Unit * caster) const {
    ...
    case SPELL_AURA_PERIODIC_DAMAGE_PERCENT: {
        ...
        if (GetSpellProto()->Effect[GetEffIndex()]
                ...
                && caster->SpellHitResult(target, GetSpellProto(), false)
                        != SPELL_MISS_NONE)
            break;
        ...
        if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
        {
            switch (GetId())
            {
                ...
                case 589: // Shadow Word: Pain
                    if (AuraEffect* aurEff = caster->GetDummyAuraEffect(SPELLFAMILY_PRIEST, 4879, 0))
                    {
        ...
        if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE) {
            damage += caster->SpellDamageBonus(target, GetSpellProto(),
                    GetEffIndex(), damage, DOT, GetBase()->GetStackAmount());

            // Calculate armor mitigation
            if (Unit::IsDamageReducedByArmor(
                    GetSpellSchoolMask(GetSpellProto()), GetSpellProto(),
                    m_effIndex)) {
                uint32 damageReductedArmor = caster->CalcArmorReducedDamage(
                        target, damage, GetSpellProto());
            ...
            if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_WARLOCK
                    && (GetSpellProto()->SpellFamilyFlags[0] & 0x400)
            ...
            if (GetSpellProto()->Id == 1978)
                damage = int32((
                        GetBase()->GetCaster()->GetTotalAttackPowerValue(
                                RANGED_ATTACK) * 0.4f + (damage * 15 / 3))/5);
        ...
        if (crit)
            damage = caster->SpellCriticalDamageBonus(m_spellProto, damage,
                    target);
        ...
        caster->CalcAbsorbResist(target, GetSpellSchoolMask(GetSpellProto()),
                DOT, damage, &absorb, &resist, m_spellProto);

        sLog->outDetail(
                "PeriodicTick: %u (TypeId: %u) attacked %u (TypeId: %u) for %u dmg inflicted by %u abs is %u",
                GUID_LOPART(GetCasterGUID()),
                GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())),
                target->GetGUIDLow(), target->GetTypeId(), damage, GetId(),
                absorb);
        ...
        SpellPeriodicAuraLogInfo pInfo(this, damage, overkill, absorb, resist,
                0.0f, crit);
        target->SendPeriodicAuraLog(&pInfo);

        caster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx,
                damage, BASE_ATTACK, GetSpellProto());

        caster->DealDamage(target, damage, &cleanDamage, DOT,
    ...
    case SPELL_AURA_PERIODIC_LEECH: {
        ...
        uint32 absorb = 0;
        uint32 resist = 0;
        CleanDamage cleanDamage = CleanDamage(0, 0, BASE_ATTACK,
                MELEE_HIT_NORMAL);
        ...
        // Calculate armor mitigation
        if (Unit::IsDamageReducedByArmor(GetSpellSchoolMask(GetSpellProto()),
                GetSpellProto(), m_effIndex)) {
            uint32 damageReductedArmor = caster->CalcArmorReducedDamage(target,
                    damage, GetSpellProto());
            cleanDamage.mitigated_damage += damage - damageReductedArmor;
            damage = damageReductedArmor;
        }
        ...
        caster->SendSpellNonMeleeDamageLog(target, GetId(), damage,
                GetSpellSchoolMask(GetSpellProto()), absorb, resist, false, 0,
                crit);
        ...
        caster->ProcDamageAndSpell(target, procAttacker, procVictim, procEx,
                damage, BASE_ATTACK, GetSpellProto());
        int32 new_damage = caster->DealDamage(target, damage, &cleanDamage, DOT,
        ...
        float gainMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(
                GetSpellProto(), GetEffIndex(), caster);

        uint32 heal = uint32(
                caster->SpellHealingBonus(caster, GetSpellProto(),
                        GetEffIndex(), uint32(new_damage * gainMultiplier), DOT,
                        GetBase()->GetStackAmount()));

        int32 gain = caster->HealBySpell(caster, GetSpellProto(), heal);
        caster->getHostileRefManager().threatAssist(caster, gain * 0.5f,
                GetSpellProto());
        break;
    }
    case SPELL_AURA_PERIODIC_HEALTH_FUNNEL: // only three spells
    {
        ...
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
                "PeriodicTick: donator %u target %u damage %u.",
                target->GetEntry(), target->GetEntry(), damage);
    ...
    case SPELL_AURA_OBS_MOD_HEALTH: {
        ...
        if (target != caster
                && GetSpellProto()->AttributesEx2 & SPELL_ATTR2_HEALTH_FUNNEL
                && !caster->isAlive())
            break;
        ...
        if (GetAuraType() == SPELL_AURA_OBS_MOD_HEALTH) {
            ...
            float minval = (float) target->GetMaxNegativeAuraModifier(
                    SPELL_AURA_MOD_HEALING_PCT);
            if (minval)
                TakenTotalMod *= (100.0f + minval) / 100.0f;

            float maxval = (float) target->GetMaxPositiveAuraModifier(
                    SPELL_AURA_MOD_HEALING_PCT);
            if (maxval)
                TakenTotalMod *= (100.0f + maxval) / 100.0f;

            // Healing over time taken percent
            float minval_hot = (float) target->GetMaxNegativeAuraModifier(
                    SPELL_AURA_MOD_HOT_PCT);
            if (minval_hot)
                TakenTotalMod *= (100.0f + minval_hot) / 100.0f;

            float maxval_hot = (float) target->GetMaxPositiveAuraModifier(
                    SPELL_AURA_MOD_HOT_PCT);
            if (maxval_hot)
        ...
        default:
            ...
            if (m_spellProto->SpellFamilyName == SPELLFAMILY_DRUID
                    && m_spellProto->SpellIconID == 2864)
                addition =
                        int32(
                                float(damage * GetTotalTicks())
                                        * ((6 - float(2 * (GetTickNumber() - 1)))
                                                / 100));
            ...
            damage += addition;
            damage = caster->SpellHealingBonus(target, GetSpellProto(),
                    GetEffIndex(), damage, DOT, GetBase()->GetStackAmount());
        }
        switch (m_spellProto->Id) {
        case 774: {
        ...
        case 29841: // Second Wind r1
            damage = int32(caster->GetMaxHealth() * 0.004f);
            break;
        ...
        bool crit = IsPeriodicTickCrit(target, caster);
        if (crit)
            damage = caster->SpellCriticalHealingBonus(m_spellProto, damage,
                    target);
        ...
        SpellPeriodicAuraLogInfo pInfo(this, damage, damage - gain, absorb, 0,
                0.0f, crit);
        target->SendPeriodicAuraLog(&pInfo);

        target->getHostileRefManager().threatAssist(caster, float(gain) * 0.5f,
                GetSpellProto());
        ...
        if (target != caster
                && GetSpellProto()->AttributesEx2 & SPELL_ATTR2_HEALTH_FUNNEL) {
            uint32 damage = SpellMgr::CalculateSpellEffectAmount(
                    GetSpellProto(), 0); // damage is not affected by spell power
            if ((int32) damage > gain)
                damage = gain;
            uint32 absorb = 0;
            caster->DealDamageMods(caster, damage, &absorb);
            caster->SendSpellNonMeleeDamageLog(caster, GetId(), damage,
                    GetSpellSchoolMask(GetSpellProto()), absorb, 0, false, 0,
                    false);

            CleanDamage cleanDamage = CleanDamage(0, 0, BASE_ATTACK,
                    MELEE_HIT_NORMAL);
            caster->DealDamage(caster, damage, &cleanDamage, NODAMAGE,
                    GetSpellSchoolMask(GetSpellProto()), GetSpellProto(), true);
        }
    ...
    case SPELL_AURA_PERIODIC_MANA_LEECH: {
        ...
        int32 drain_amount =
                target->GetPower(power) > damage ?
                        damage : target->GetPower(power);
        ...
        if (caster->GetMaxPower(power) > 0)
            gain_multiplier = SpellMgr::CalculateSpellEffectValueMultiplier(
                    GetSpellProto(), GetEffIndex(), caster);

        SpellPeriodicAuraLogInfo pInfo(this, drain_amount, 0, 0, 0,
                gain_multiplier, false);
        target->SendPeriodicAuraLog(&pInfo);
        ...
        if (gain_amount) {
            ...
            target->AddThreat(caster, float(gain) * 0.5f,
                    GetSpellSchoolMask(GetSpellProto()), GetSpellProto());
        }
        ...
        if (m_spellProto->SpellFamilyName == SPELLFAMILY_WARLOCK
            ...
            if (manaFeedVal > 0) {
                ...
                caster->CastCustomSpell(caster, 32554, &manaFeedVal, NULL, NULL,
                        true, NULL, this);
            }
        }
        break;
    ...
    case SPELL_AURA_OBS_MOD_POWER: {
        ...
        if (GetBase()->GetDuration() == -1
                && target->GetPower(power) == target->GetMaxPower(power))
            return;
        ...
        sLog->outDetail(
                ...
                GUID_LOPART(GetCasterGUID()),
                GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())),
                target->GetGUIDLow(), target->GetTypeId(), amount, GetId());
        ...
        if (caster)
            target->getHostileRefManager().threatAssist(caster,
                    float(gain) * 0.5f, GetSpellProto());
        break;
    }
    case SPELL_AURA_PERIODIC_ENERGIZE: {
        // ignore non positive values (can be result apply spellmods to aura damage
    ...
    case SPELL_AURA_POWER_BURN: {
        ...
        SpellNonMeleeDamage damageInfo(caster, target, spellProto->Id,
                spellProto->SchoolMask);
        //no SpellDamageBonus for burn mana
        caster->CalculateSpellDamageTaken(&damageInfo,
                int32(gain * dmgMultiplier), spellProto);
        ...
        caster->ProcDamageAndSpell(damageInfo.target, procAttacker, procVictim,
                procEx, damageInfo.damage, BASE_ATTACK, spellProto);
...
void AuraEffect::PeriodicDummyTick(Unit *target, Unit *caster) const {
    ...
    case SPELLFAMILY_GENERIC:
        ...
        case 62292: // Blaze (Pool of Tar)
            ...
            target->CastSpell((Unit*) NULL,
                    m_spellProto->EffectTriggerSpell[m_effIndex], true);
            break;
        case 62399: // Overload Circuit
            if (target->GetMap()->IsDungeon()
                    && int(target->GetAppliedAuras().count(62399))
                            >= (target->GetMap()->IsHeroic() ? 4 : 2)) {
        ...
        case 64821: // Fuse Armor (Razorscale)
            if (GetBase()->GetStackAmount() == GetSpellProto()->StackAmount) {
                target->CastSpell(target, 64774, true, NULL, NULL,
                        GetCasterGUID());
    ...
    case SPELLFAMILY_DRUID: {
        ...
        case 22842: {
            ...
            int32 points = target->CalculateSpellDamage(target, GetSpellProto(),
                    1);
            int32 regen = (target->GetMaxHealth() * (mod * points / 10) / 100)
                    / 200;
    ...
    case SPELLFAMILY_ROGUE: {
        ...
        case 51690: {
            ...
            UnitList targets;
            {
                ...
                CellPair p(
                        Trinity::ComputeCellPair(caster->GetPositionX(),
                                caster->GetPositionY()));
                Cell cell(p);
                cell.data.Part.reserved = ALL_DISTRICT;

                Trinity::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(
                        caster, caster, radius);
                Trinity::UnitListSearcher<
                        Trinity::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(
                        caster, targets, u_check);

                TypeContainerVisitor<
                        Trinity::UnitListSearcher<
                                Trinity::AnyUnfriendlyVisibleUnitInObjectRangeCheck>,
                        GridTypeMapContainer> grid_object_checker(checker);
                TypeContainerVisitor<
                        Trinity::UnitListSearcher<
                                Trinity::AnyUnfriendlyVisibleUnitInObjectRangeCheck>,
                        WorldTypeMapContainer> world_object_checker(checker);

                cell.Visit(p, grid_object_checker,
                        *GetBase()->GetOwner()->GetMap(), *caster, radius);
                cell.Visit(p, world_object_checker,
                        *GetBase()->GetOwner()->GetMap(), *caster, radius);
            }
    ...
    case SPELLFAMILY_HUNTER: {
        ...
        if (GetSpellProto()->SpellFamilyFlags[1] & 0x80000000) {
            if (caster)
                caster->CastCustomSpell(53352, SPELLVALUE_BASE_POINT0, m_amount,
                        target, true, NULL, this);
            break;
        }
        switch (GetSpellProto()->Id) {
    ...
    case SPELLFAMILY_SHAMAN:
        if (GetId() == 52179) // Astral Shift
            ...
            if (!(target->GetUInt32Value(UNIT_FIELD_FLAGS)
    ...
    case SPELLFAMILY_DEATHKNIGHT:
        ...
        if (GetSpellProto()->SpellFamilyFlags[0] & 0x20) {
            if (caster)
                caster->CastCustomSpell(target, 52212, &m_amount, NULL, NULL,
                        true, 0, this);
            break;
        }
        // Blood of the North
        // Reaping
        // Death Rune Mastery
        if (GetSpellProto()->SpellIconID == 3041
                || GetSpellProto()->SpellIconID == 22
                || GetSpellProto()->SpellIconID == 2622) {
            if (target->GetTypeId() != TYPEID_PLAYER)
                return;
...
void AuraEffect::TriggerSpell(Unit *target, Unit *caster) const {
    ...
    SpellEntry const *triggeredSpellInfo = sSpellStore.LookupEntry(
            triggerSpellId);
    ...
    if (triggeredSpellInfo == NULL) {
        ...
        case SPELLFAMILY_GENERIC: {
            ...
            case 27746:
                ...
                // Frost Blast
            case 27808:
                caster->CastCustomSpell(29879, SPELLVALUE_BASE_POINT0,
                        int32(target->CountPctFromMaxHealth(21)), target, true,
                        NULL, this);
                return;
                // Detonate Mana
            case 27819:
                if (int32 mana = (int32)(target->GetMaxPower(POWER_MANA) / 10)) {
                    ...
                    target->CastCustomSpell(27820, SPELLVALUE_BASE_POINT0,
                            -mana * 10, target, true, NULL, this);
                }
                return;
            ...
            case 30427: {
                ...
                if (caster->GetTypeId() == TYPEID_PLAYER
                        ...
                        && triggerTarget->ToCreature()->GetCreatureInfo()->type
                    ...
                    player->AutoStoreLoot(
                            creature->GetCreatureInfo()->SkinLootId,
                            LootTemplates_Skinning, true);
            ...
            case 31373: {
                ...
                target->SummonCreature(17870, 0, 0, 0, target->GetOrientation(),
            ...
            case 40879:
                switch (rand() % 6) {
                case 0:
                    triggerSpellId = 40880;
                    break;
                case 1:
                    triggerSpellId = 40882;
                    break;
                case 2:
                    triggerSpellId = 40883;
                    break;
                case 3:
                    triggerSpellId = 40891;
                    break;
                case 4:
                    triggerSpellId = 40896;
                    break;
                case 5:
                    triggerSpellId = 40897;
                    break;
                }
                break;
                // Aura of Desire
            case 41350: {
                ...
                int32 amount =
                        aurEff->GetAmount() - 5 < -100 ?
                                -100 : aurEff->GetAmount() - 5;
        ...
        case SPELLFAMILY_SHAMAN: {
            ...
            case 28820: {
                ...
                if (!target->GetAuraEffect(SPELL_AURA_PROC_TRIGGER_SPELL,
                        SPELLFAMILY_SHAMAN, 0x400))
    ...
    if (triggeredSpellInfo) {
        Unit *triggerCaster = GetTriggeredSpellCaster(triggeredSpellInfo,
                caster, triggerTarget);
        triggerCaster->CastSpell(triggerTarget, triggeredSpellInfo, true, 0,
                this);
        sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
                "AuraEffect::TriggerSpell: Spell %u Trigger %u", GetId(),
                triggeredSpellInfo->Id);
    } else {
        Creature* c = triggerTarget->ToCreature();
        if (!c
                || (c
                        && !sScriptMgr->OnDummyEffect(caster, GetId(),
                                SpellEffIndex(GetEffIndex()),
                                triggerTarget->ToCreature()))
                || (c
                        && !c->AI()->sOnDummyEffect(caster, GetId(),
                                SpellEffIndex(GetEffIndex()))))
            sLog->outDebug(
                    LOG_FILTER_SPELLS_AURAS,
                    "AuraEffect::TriggerSpell: Spell %u has value 0 in EffectTriggered[%d] and is therefor not handled. Define as custom case?",
                    GetId(), GetEffIndex());
    }
}
...
void AuraEffect::TriggerSpellWithValue(Unit *target, Unit *caster) const {
    ...
    if (triggeredSpellInfo) {
        ...
        triggerCaster->CastCustomSpell(triggerTarget, triggerSpellId,
                &basepoints0, 0, 0, true, 0, this);
    } else
        sLog->outError(
                "AuraEffect::TriggerSpellWithValue: Spell %u have 0 in EffectTriggered[%d], not handled custom case?",
                GetId(), GetEffIndex());
}
...
bool AuraEffect::IsAffectedOnSpell(SpellEntry const *spell) const {
    ...
    if (m_spellProto->EffectSpellClassMask[m_effIndex]
...
void AuraEffect::CleanupTriggeredSpells(Unit *target) {
    ...
    if (m_spellProto->EffectApplyAuraName[GetEffIndex()]
            == SPELL_AURA_PERIODIC_TRIGGER_SPELL
            && uint32(GetSpellDuration(m_spellProto))
                    == m_spellProto->EffectAmplitude[GetEffIndex()])
        return;
...
void AuraEffect::HandleModInvisibility(AuraApplication const *aurApp,
    ...
    if (apply) {
        ...
        if (mode & AURA_EFFECT_HANDLE_REAL)
            target->RemoveAurasWithInterruptFlags(
                    AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION);

        // apply glow vision
        if (target->GetTypeId() == TYPEID_PLAYER)
            target->SetByteFlag(PLAYER_FIELD_BYTES2, 3,
        ...
        if (!target->HasAuraType(SPELL_AURA_MOD_INVISIBILITY)) {
            ...
            // remove glow vision
            if (target->GetTypeId() == TYPEID_PLAYER)
                target->RemoveByteFlag(PLAYER_FIELD_BYTES2, 3,
                        PLAYER_FIELD_BYTE2_INVISIBILITY_GLOW);
...
void AuraEffect::HandleModCamouflage(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandleAuraGhost(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandlePhase(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    ...
    uint32 newPhase = 0;
    Unit::AuraEffectList const& phases = target->GetAuraEffectsByType(
            SPELL_AURA_PHASE);
    if (!phases.empty())
        for (Unit::AuraEffectList::const_iterator itr = phases.begin();
                itr != phases.end(); ++itr)
        ...
        if (PowerType != POWER_MANA) {
            ...
            case FORM_DIREBEAR: {
                ...
                case FORM_CAT: {
                    ...
                    target->CastCustomSpell(target, 17099, &basePoints, NULL,
                            NULL, true, NULL, this);
                }
                    break;
                case FORM_BEAR:
                ...
                default: {
                    uint32 newEnergy = std::min(target->GetPower(POWER_ENERGY),
                            FurorChance);
            ...
            if (target->GetTypeId() == TYPEID_PLAYER) {
                ...
                for (PlayerSpellMap::const_iterator itr = sp_list.begin();
                    ...
                    if (spellInfo
                        ...
                        Rage_val += target->CalculateSpellDamage(target,
                                spellInfo, 0) * 10;
                }
            }
    ...
    if (target->getClass() == CLASS_DRUID) {
        ...
        if (!target->CanUseAttackType(BASE_ATTACK)) {
            if (Item *pItem = target->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_MAINHAND)) {
                target->ToPlayer()->_ApplyWeaponDamage(EQUIPMENT_SLOT_MAINHAND,
                        pItem->GetProto(), NULL, apply);
            }
        }
    }
    ...
    if (target->GetTypeId() == TYPEID_PLAYER) {
        ...
        for (uint8 i = 0; i < MAX_SHAPESHIFT_SPELLS; ++i) {
            if (!shapeInfo->stanceSpell[i])
                continue;
            if (apply)
                target->ToPlayer()->AddTemporarySpell(
                        shapeInfo->stanceSpell[i]);
            else
                target->ToPlayer()->RemoveTemporarySpell(
                        shapeInfo->stanceSpell[i]);
        }
    }
}

void AuraEffect::HandleAuraTransform(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
            ...
            if (GetMiscValue() == 0) {
                ...
                case 16739: {
                    ...
                    case 16125:
                        ...
                        // Worgen Female
                    case 29423:
                        target->SetDisplayId(30114);
                        break;
                        // Worgen Male
                    case 29422:
                        target->SetDisplayId(29335);
                        break;
                        // Goblin Female
                    case 6895:
                        target->SetDisplayId(33613);
                        break;
                        // Goblin Male
                    case 6894:
                        target->SetDisplayId(33611);
                        break;
                    default:
                        break;
                    }
                ...
                if (!ci) {
                    ...
                    sLog->outError(
                            "Auras: unknown creature id = %d (only need its modelid) From Spell Aura Transform in Spell ID = %d",
                            GetMiscValue(), GetId());
                } else {
                    uint32 model_id = 0;
                    ...
                    // Polymorph (sheep)
                    if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_MAGE
                            && GetSpellProto()->SpellIconID == 82
                            && GetSpellProto()->SpellVisual[0] == 12978)
                    ...
                    if (GetId() == 42016 && target->GetMountID()
                            && !target->GetAuraEffectsByType(
                                    SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED).empty())
                        target->SetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID,
                                16314);
                }
            }
        }
            ...
            if (!target->GetAuraEffectsByType(SPELL_AURA_MOUNTED).empty()) {
                ...
                if (CreatureInfo const* ci = ObjectMgr::GetCreatureTemplate(cr_id)) {
                    ...
                    CreatureModelInfo const *minfo =
                            sObjectMgr->GetCreatureModelRandomGender(
                                    display_id);
                    if (minfo)
...
void AuraEffect::HandleAuraModScale(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    ...
    target->ApplyPercentModFloatValue(OBJECT_FIELD_SCALE_X, (float) GetAmount(),
            apply);
}
...
void AuraEffect::HandleFeignDeath(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    if (apply) {
        UnitList targets;
        Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target,
                target->GetMap()->GetVisibilityRange());
        Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(
                target, targets, u_check);
        target->VisitNearbyObject(target->GetMap()->GetVisibilityRange(),
                searcher);
        for (UnitList::iterator iter = targets.begin(); iter != targets.end();
                ++iter) {
            ...
            for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL;
                ...
                if ((*iter)->GetCurrentSpell(i)
                        && (*iter)->GetCurrentSpell(i)->m_targets.getUnitTargetGUID()
                                == target->GetGUID())
...
void AuraEffect::HandleAuraModDisarm(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    if (apply ?
            target->GetAuraEffectsByType(type).size() > 1 :
            target->HasAuraType(type))
        return;
    ...
    if (target->GetTypeId() == TYPEID_PLAYER && !target->IsInFeralForm()) {
        if (Item *pItem = target->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, slot)) {
            ...
            if (attacktype < MAX_ATTACK)
                target->ToPlayer()->_ApplyWeaponDamage(slot, pItem->GetProto(),
                        NULL, !apply);
        }
    }

    if (target->GetTypeId() == TYPEID_UNIT
            && target->ToCreature()->GetCurrentEquipmentId())
        target->UpdateDamagePhysical(attType);
}

void AuraEffect::HandleAuraModSilence(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleAuraModPacify(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandleAuraTrackStealthed(AuraApplication const *aurApp,
    ...
    target->ApplyModFlag(PLAYER_FIELD_BYTES, PLAYER_FIELD_BYTE_TRACK_STEALTHED,
            apply);
}

void AuraEffect::HandleAuraModStalked(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandleAuraUntrackable(AuraApplication const *aurApp,
    ...
    if (apply)
        ...
        if (target->HasAuraType(GetAuraType()))
            return;
        target->RemoveByteFlag(UNIT_FIELD_BYTES_1, 3,
...
void AuraEffect::HandleAuraModSkill(AuraApplication const *aurApp,
    ...
    target->ToPlayer()->ModifySkillBonus(prot, ((apply) ? points : -points),
            GetAuraType() == SPELL_AURA_MOD_SKILL_TALENT);
...
void AuraEffect::HandleAuraMounted(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
    ...
    if (apply) {
        ...
        if (aurApp->GetBase()->GetId() == 87840) {
            target->Mount(plr->getGender() == GENDER_FEMALE ? 29423 : 29422, 0,
                    GetMiscValue());
            target->Mount(plr->getGender() == GENDER_MALE ? 29422 : 29423, 0,
                    GetMiscValue());
            return;
        }
        ...
        if (!ci) {
            sLog->outErrorDb(
                    "AuraMounted: `creature_template`='%u' not found in database (only need it modelid)",
                    GetMiscValue());
            return;
        }
...
void AuraEffect::HandleAuraWaterWalk(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandleAuraHover(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandleWaterBreathing(AuraApplication const *aurApp, uint8 mode,
        bool /*apply*/) const {
    if (!(mode & AURA_EFFECT_HANDLE_SEND_FOR_CLIENT_MASK))
        return;
...
void AuraEffect::HandleModThreat(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK))
        return;
    ...
    for (int32 i = 0; i < MAX_SPELL_SCHOOL; ++i)
        if (GetMiscValue() & (1 << i))
            ApplyPercentModFloatVar(target->m_threatModifier[i],
...
void AuraEffect::HandleModTaunt(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleModConfuse(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleModFear(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleAuraModStun(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandlePreventFleeing(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    Unit::AuraEffectList const& fearAuras = target->GetAuraEffectsByType(
            SPELL_AURA_MOD_FEAR);
    if (!fearAuras.empty())
        target->SetControlled(!(apply), UNIT_STAT_FLEEING);
}
...
void AuraEffect::HandleModPossess(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleModPossessPet(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    if (apply) {
        ...
        if (!pet->IsWithinDistInMap(caster,
                pet->GetMap()->GetVisibilityRange()))
            pet->Remove(PET_SLOT_OTHER_PET, true);
        else {
            // Reinitialize the pet bar and make the pet come back to the owner
            caster->ToPlayer()->PetSpellInitialize();
            if (!pet->getVictim())
                pet->GetMotionMaster()->MoveFollow(caster, PET_FOLLOW_DIST,
                        pet->GetFollowAngle());
        }
    }
}

void AuraEffect::HandleModCharm(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleCharmConvert(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleAuraModIncreaseMountedSpeed(
...
void AuraEffect::HandleAuraModIncreaseFlightSpeed(AuraApplication const *aurApp,
    ...
    if (GetAuraType() == SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED) {
        ...
        if (mode & AURA_EFFECT_HANDLE_REAL) {
            ...
            if (target->GetTypeId() == TYPEID_PLAYER)
                target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC,
                        MECHANIC_POLYMORPH, apply);

            // Dragonmaw Illusion (overwrite mount model, mounted aura already applied)
            if (apply && target->HasAuraEffect(42016, 0)
                    && target->GetMountID())
...
void AuraEffect::HandleModStateImmunityMask(AuraApplication const *aurApp,
    ...
    if (apply
        ...
        for (std::list<AuraType>::iterator iter = immunity_list.begin();
                iter != immunity_list.end(); ++iter)
...
void AuraEffect::HandleModMechanicImmunity(AuraApplication const *aurApp,
    ...
    case 54508: // Demonic Empowerment
        ...
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SNARE,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_ROOT,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_STUN,
                apply);
        break;
    ...
    case 19574: // Bestial Wrath
        ...
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_ROOT,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_FEAR,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_STUN,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SLEEP,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_CHARM,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_SAPPED,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_HORROR,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_POLYMORPH,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC,
                MECHANIC_DISORIENTED, apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_FREEZE,
                apply);
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, MECHANIC_TURN,
                apply);
        break;
    default:
        if (GetMiscValue() < 1)
        ...
        target->ApplySpellImmune(GetId(), IMMUNITY_MECHANIC, GetMiscValue(),
                apply);
        break;
    }
    ...
    if (apply
            && GetSpellProto()->AttributesEx
                    & SPELL_ATTR1_DISPEL_AURAS_ON_IMMUNITY)
        target->RemoveAurasWithMechanic(mechanic, AURA_REMOVE_BY_DEFAULT,
                GetId());
}
    ...
    if (!(apply) && target->GetTypeId() == TYPEID_PLAYER
        ...
        if (target->GetTypeId() == TYPEID_PLAYER) {
            if (target->ToPlayer()->InBattleground()) {
                ...
                sOutdoorPvPMgr->HandleDropFlag((Player*) target,
                        GetSpellProto()->Id);
        }
    }
    // stop handling the effect if it was removed by linked event
...
void AuraEffect::HandleAuraModSchoolImmunity(AuraApplication const *aurApp,
    ...
    if (apply
        ...
        for (Unit::AuraApplicationMap::iterator iter = Auras.begin();
            ...
            if ((GetSpellSchoolMask(spell) & school_mask)
                    ...
                    && !iter->second->IsPositive() && spell->Id != GetId())
    ...
    if (GetSpellProto()->Mechanic == MECHANIC_BANISH) {
        if (apply)
            ...
            Unit::AuraEffectList const& banishAuras =
                    target->GetAuraEffectsByType(GetAuraType());
            for (Unit::AuraEffectList::const_iterator i = banishAuras.begin();
                    i != banishAuras.end(); ++i)
...
void AuraEffect::HandleAuraModDispelImmunity(AuraApplication const *aurApp,
    ...
    target->ApplySpellDispelImmunity(m_spellProto, DispelType(GetMiscValue()),
            apply);
}
...
void AuraEffect::HandleAuraModResistanceExclusive(AuraApplication const *aurApp,
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++) {
        if (GetMiscValue() & int32(1 << x)) {
            int32 amount = target->GetMaxPositiveAuraModifierByMiscMask(
                    SPELL_AURA_MOD_RESISTANCE_EXCLUSIVE, 1 << x, this);

            if (amount < modAmount) {
                target->HandleStatModifier(
                        UnitMods(UNIT_MOD_RESISTANCE_START + x), BASE_VALUE,
                        float(modAmount - amount), apply);
                if (target->GetTypeId() == TYPEID_PLAYER)
                    target->ApplyResistanceBuffModsMod(SpellSchools(x),
                            aurApp->IsPositive(), float(modAmount - amount),
                            apply);
            }
        }
    }
...
void AuraEffect::HandleAuraModResistance(AuraApplication const *aurApp,
    ...
    for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++) {
        if (GetMiscValue() & int32(1 << x)) {
            target->HandleStatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + x),
                    TOTAL_VALUE, float(GetAmount()), apply);
            if (target->GetTypeId() == TYPEID_PLAYER
                    || target->ToCreature()->isPet())
                target->ApplyResistanceBuffModsMod(SpellSchools(x),
                        GetAmount() > 0, float(GetAmount()), apply);
        }
    }
}
...
void AuraEffect::HandleAuraModBaseResistancePCT(AuraApplication const *aurApp,
    ...
    if (target->GetTypeId() != TYPEID_PLAYER) {
        ...
        if (target->ToCreature()->isPet()
                && (GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
            target->HandleStatModifier(UNIT_MOD_ARMOR, BASE_PCT,
                    float(GetAmount()), apply);
    } else {
        for (int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL; x++)
            if (GetMiscValue() & int32(1 << x))
...
void AuraEffect::HandleModResistancePercent(AuraApplication const *aurApp,
    ...
    for (int8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; i++) {
        if (GetMiscValue() & int32(1 << i)) {
            ...
            if (target->GetTypeId() == TYPEID_PLAYER
                    || target->ToCreature()->isPet()) {
                target->ApplyResistanceBuffModsPercentMod(SpellSchools(i), true,
                        (float) GetAmount(), apply);
                target->ApplyResistanceBuffModsPercentMod(SpellSchools(i),
                        false, (float) GetAmount(), apply);
            }
        }
    }
}
...
void AuraEffect::HandleModBaseResistance(AuraApplication const *aurApp,
    ...
    if (target->GetTypeId() != TYPEID_PLAYER) {
        ...
        if (target->ToCreature()->isPet()
                && (GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
            target->HandleStatModifier(UNIT_MOD_ARMOR, TOTAL_VALUE,
                    float(GetAmount()), apply);
    } else {
...
void AuraEffect::HandleModTargetResistance(AuraApplication const *aurApp,
    ...
    if (target->GetTypeId() == TYPEID_PLAYER
            && (GetMiscValue() & SPELL_SCHOOL_MASK_SPELL)
                    == SPELL_SCHOOL_MASK_SPELL)
        target->ApplyModInt32Value(PLAYER_FIELD_MOD_TARGET_RESISTANCE,
                GetAmount(), apply);
}
...
void AuraEffect::HandleModPercentStat(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    for (int32 i = STAT_STRENGTH; i < MAX_STATS; ++i)
        if (GetMiscValue() == i || GetMiscValue() == -1)
            target->HandleStatModifier(UnitMods(UNIT_MOD_STAT_START + i),
                    BASE_PCT, float(m_amount), apply);
}

void AuraEffect::HandleModSpellDamagePercentFromStat(
        AuraApplication const *aurApp, uint8 mode, bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleModSpellHealingPercentFromStat(
        AuraApplication const *aurApp, uint8 mode, bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleModSpellDamagePercentFromAttackPower(
        AuraApplication const *aurApp, uint8 mode, bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleModSpellHealingPercentFromAttackPower(
        AuraApplication const *aurApp, uint8 mode, bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraModSpellPowerPercent(AuraApplication const * aurApp,
        uint8 mode, bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleModHealingDone(AuraApplication const *aurApp, uint8 mode,
        bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraModResistenceOfStatPercent(
        AuraApplication const *aurApp, uint8 mode, bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
/***      HEAL & ENERGIZE     ***/
/********************************/
void AuraEffect::HandleModPowerRegen(AuraApplication const *aurApp, uint8 mode,
        bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleModManaRegen(AuraApplication const *aurApp, uint8 mode,
        bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraModIncreaseMaxHealth(AuraApplication const *aurApp,
    ...
    double healthPercentage = (double) oldhealth
            / (double) target->GetMaxHealth();

    target->HandleStatModifier(UNIT_MOD_HEALTH, TOTAL_VALUE, float(GetAmount()),
            apply);

    // refresh percentage
    if (oldhealth > 0) {
        uint32 newhealth = uint32(
                ceil((double) target->GetMaxHealth() * healthPercentage));
        if (newhealth == 0)
            newhealth = 1;
...
void AuraEffect::HandleAuraModIncreaseEnergy(AuraApplication const *aurApp,
    ...
    if (GetId() == 64904) // Hymn of Hope
            {
        if (mode & AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK) {
            int32 change = target->GetPower(powerType)
                    + (apply ? GetAmount() : -GetAmount());
            if (change < 0)
                change = 0;
...
void AuraEffect::HandleAuraModIncreaseEnergyPercent(
        AuraApplication const *aurApp, uint8 mode, bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraIncreaseBaseHealthPercent(
        AuraApplication const *aurApp, uint8 mode, bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraModWeaponCritPercent(AuraApplication const *aurApp,
    ...
    for (int i = 0; i < MAX_ATTACK; ++i)
        if (Item* pItem = target->ToPlayer()->GetWeaponForAttack(WeaponAttackType(i), true))
            target->ToPlayer()->_ApplyWeaponDependentAuraCritMod(pItem,
                    WeaponAttackType(i), this, apply);
    ...
    if (GetSpellProto()->EquippedItemClass == -1) {
        target->ToPlayer()->HandleBaseModValue(CRIT_PERCENTAGE, FLAT_MOD,
                float(GetAmount()), apply);
        target->ToPlayer()->HandleBaseModValue(OFFHAND_CRIT_PERCENTAGE,
                FLAT_MOD, float(GetAmount()), apply);
        target->ToPlayer()->HandleBaseModValue(RANGED_CRIT_PERCENTAGE, FLAT_MOD,
                float(GetAmount()), apply);
    }
}

void AuraEffect::HandleModHitChance(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraModCritPct(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    target->ToPlayer()->HandleBaseModValue(CRIT_PERCENTAGE, FLAT_MOD,
            float(GetAmount()), apply);
    target->ToPlayer()->HandleBaseModValue(OFFHAND_CRIT_PERCENTAGE, FLAT_MOD,
            float(GetAmount()), apply);
    target->ToPlayer()->HandleBaseModValue(RANGED_CRIT_PERCENTAGE, FLAT_MOD,
            float(GetAmount()), apply);
...
void AuraEffect::HandleModMeleeRangedSpeedPct(AuraApplication const *aurApp,
    ...
    target->ApplyAttackTimePercentMod(RANGED_ATTACK, (float) GetAmount(),
            apply);
}
...
void AuraEffect::HandleModAttackSpeed(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
...
void AuraEffect::HandleAuraModAttackPower(AuraApplication const *aurApp,
    ...
    if (float(GetAmount()) > 0.f)
        target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_POS, TOTAL_VALUE,
                float(GetAmount()), apply);
    else
        target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_NEG, TOTAL_VALUE,
                -float(GetAmount()), apply);
}
...
void AuraEffect::HandleAuraModAttackPowerPercent(AuraApplication const *aurApp,
    ...
    if (float(GetAmount()) > 0.f)
        target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_POS, TOTAL_PCT,
                float(GetAmount()), apply);
    else
        target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_NEG, TOTAL_PCT,
                -float(GetAmount()), apply);
}

void AuraEffect::HandleAuraModRangedAttackPowerPercent(
        AuraApplication const *aurApp, uint8 mode, bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    if (float(GetAmount()) > 0.f)
        target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED_POS, TOTAL_PCT,
                float(GetAmount()), apply);
    else
        target->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED_NEG, TOTAL_PCT,
                -float(GetAmount()), apply);
}

void AuraEffect::HandleAuraModRangedAttackPowerOfStatPercent(
        AuraApplication const *aurApp, uint8 mode, bool /*apply*/) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    // Recalculate bonus
    if (target->GetTypeId() == TYPEID_PLAYER
            && !(target->getClassMask() & CLASSMASK_WAND_USERS))
        target->ToPlayer()->UpdateAttackPowerAndDamage(true);
}

void AuraEffect::HandleAuraModAttackPowerOfStatPercent(
        AuraApplication const * aurApp, uint8 mode, bool apply) const {
...
/***        DAMAGE BONUS      ***/
/********************************/
void AuraEffect::HandleModDamageDone(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode
            & (AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK | AURA_EFFECT_HANDLE_STAT)))
        return;
    ...
    if (target->GetTypeId() == TYPEID_PLAYER) {
        for (int i = 0; i < MAX_ATTACK; ++i)
            if (Item* pItem = target->ToPlayer()->GetWeaponForAttack(WeaponAttackType(i), true))
                target->ToPlayer()->_ApplyWeaponDependentAuraDamageMod(pItem,
                        WeaponAttackType(i), this, apply);
    }
            ...
            if (target->GetTypeId() == TYPEID_PLAYER) {
                if (GetAmount() > 0)
                    target->ApplyModUInt32Value(
                            PLAYER_FIELD_MOD_DAMAGE_DONE_POS, GetAmount(),
                            apply);
                else
                    target->ApplyModUInt32Value(
                            PLAYER_FIELD_MOD_DAMAGE_DONE_NEG, -GetAmount(),
                            apply);
            }
        }
    }
        ...
        if (GetAmount() > 0) {
            for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; i++) {
                if ((GetMiscValue() & (1 << i)) != 0)
                    target->ApplyModUInt32Value(
                            PLAYER_FIELD_MOD_DAMAGE_DONE_POS + i, GetAmount(),
                            apply);
            }
        } else {
            for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; i++) {
                if ((GetMiscValue() & (1 << i)) != 0)
                    target->ApplyModUInt32Value(
                            PLAYER_FIELD_MOD_DAMAGE_DONE_NEG + i, -GetAmount(),
                            apply);
            }
        }
...
void AuraEffect::HandleModDamagePercentDone(AuraApplication const *aurApp,
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
            "AURA MOD DAMAGE type:%u negative:%u", GetMiscValue(),
            GetAmount() > 0);
        ...
        if (GetSpellProto()->EquippedItemClass == -1
            ...
            if (target->GetTypeId() == TYPEID_PLAYER)
                target->ApplyModSignedFloatValue(
                        PLAYER_FIELD_MOD_DAMAGE_DONE_PCT, GetAmount() / 100.0f,
                        apply);
        }
    }
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
        for (int i = SPELL_SCHOOL_HOLY; i < MAX_SPELL_SCHOOL; ++i)
            target->ApplyModSignedFloatValue(
                    PLAYER_FIELD_MOD_DAMAGE_DONE_PCT + i, GetAmount() / 100.0f,
                    apply);
...
void AuraEffect::HandleShieldBlockValue(AuraApplication const *aurApp,
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
        target->ToPlayer()->HandleBaseModValue(SHIELD_BLOCK_VALUE, modType,
                float(GetAmount()), apply);
}
...
void AuraEffect::HandleModPowerCost(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK))
        return;
    ...
    for (int i = 0; i < MAX_SPELL_SCHOOL; ++i)
        if (GetMiscValue() & (1 << i))
            target->ApplyModInt32Value(UNIT_FIELD_POWER_COST_MODIFIER + i,
                    GetAmount(), apply);
}
...
void AuraEffect::HandleNoReagentUseAura(AuraApplication const *aurApp,
    ...
    flag96 mask;
    Unit::AuraEffectList const& noReagent = target->GetAuraEffectsByType(
            SPELL_AURA_NO_REAGENT_USE);
    for (Unit::AuraEffectList::const_iterator i = noReagent.begin();
            i != noReagent.end(); ++i)
...
void AuraEffect::HandleAuraDummy(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK))
        return;
    ...
    if (mode & AURA_EFFECT_HANDLE_REAL) {
        ...
        if (apply) {
            ...
            if (caster && m_spellProto->SpellFamilyName == SPELLFAMILY_WARRIOR
                ...
                if (target->IsNonMeleeSpellCasted(false, false, true, false,
                        false))
                    if (AuraEffect * aurEff = caster->GetAuraEffect(SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_WARRIOR, 2775, 0))
                        ...
                        case 46859:
                            target->CastSpell(target, 64849, true, NULL,
                                    aurEff);
                            break;
                            // Unrelenting Assault, rank 2
                        case 46860:
                            target->CastSpell(target, 64850, true, NULL,
                                    aurEff);
                            break;
                        }
            }
            ...
            case 55198: // Tidal Force
            {
                ...
                if (owner_aura) {
                    ...
                    owner_aura->SetStackAmount(
                            owner_aura->GetSpellProto()->StackAmount);
                }
                break;
            }
            case 33763:
            ...
            case 46361: // Reinforced Net
                if (caster) {
                    float currentGroundLevel = target->GetBaseMap()->GetHeight(
            ...
            case 49028:
                if (caster)
                    if (AuraEffect *aurEff = caster->GetAuraEffect(63330, 0)) // glyph of Dancing Rune Weapon
                        GetBase()->SetDuration(
                                GetBase()->GetDuration() + aurEff->GetAmount());
                break;
            ...
            case 28835: // Mark of Zeliek
                if (caster) // actually we can also use cast(this, originalcasterguid)
                {
                    ...
                    default:
                        damage = 20000
                                + 1000 * (GetBase()->GetStackAmount() - 7);
                        break;
                    }
                    if (damage)
                        caster->CastCustomSpell(28836, SPELLVALUE_BASE_POINT0,
                                damage, target);
                }
                break;
            ...
            {
                int32 mana = int32(
                        GetAmount() * pow(2.0f, GetBase()->GetStackAmount())); // mana restore - bp * 2^stackamount
                int32 damage = mana * 2; // damage

                caster->CastCustomSpell(target, 63337, &mana, NULL, NULL, true);
                caster->CastCustomSpell(target, 63338, &damage, NULL, NULL,
                        true);
                break;
            }
            case 71563: {
                if (Aura* newAura = target->AddAura(71564, target))
                    newAura->SetStackAmount(
                            newAura->GetSpellProto()->StackAmount);
                break;
            }
        ...
        // AT REMOVE
            ...
            if ((IsQuestTameSpell(GetId())) && caster && caster->isAlive()
                    && target->isAlive()) {
            ...
            case SPELLFAMILY_GENERIC:
                ...
                case 2584: // Waiting to Resurrect
                    ...
                    if (target->GetTypeId() == TYPEID_PLAYER)
                        if (Battleground *bg = target->ToPlayer()->GetBattleground())
                            bg->RemovePlayerFromResurrectQueue(
                                    target->GetGUID());
                    break;
                ...
                case 44191: // Flame Strike
                {
                    if (target->GetMap()->IsDungeon()) {
                        uint32 spellId =
                                target->GetMap()->IsHeroic() ? 46163 : 44190;
                ...
                case 60244: // Blood Parrot Despawn Aura
                    target->CastSpell((Unit*) NULL, GetAmount(), true, NULL,
                            this);
                    break;
            ...
            case SPELLFAMILY_WARLOCK: {
                ...
                if (m_spellProto->SpellFamilyFlags[1] & 0x40000) {
                    if (caster)
                        target->CastCustomSpell(caster, 48210, &m_amount, 0, 0,
                                true, NULL, this, GetCasterGUID());
                }
                break;
            }
            case SPELLFAMILY_DRUID:
            ...
            case SPELLFAMILY_PRIEST:
                ...
                if (m_spellProto->SpellFamilyFlags[1] & 0x0400 && aurApp->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL && GetEffIndex() == 0)
                {
                    ...
                    {
                        int32 chance = 0;
                        if (aurEff->GetSpellProto()->Id == 87099) chance = 50;
                        if (aurEff->GetSpellProto()->Id == 87100) chance = 100;
            ...
            case SPELLFAMILY_PALADIN: {
                ...
                // Guardian of Ancient Kings - Retribution
                case 86698: {
                    if (aurApp->GetBase()->GetOwner()->ToUnit()->HasAura(
                            86700)) {
    ...
    case SPELLFAMILY_GENERIC: {
        ...
        case 24658: {
            ...
            if (apply && caster) {
                ...
                for (uint32 i = 0; i < spell->StackAmount; ++i)
                    caster->CastSpell(target, spell->Id, true, NULL, NULL,
                            GetCasterGUID());
                break;
            }
            target->RemoveAurasDueToSpell(spellId);
        ...
        case 62061: // Festive Holiday Mount
            if (target->HasAuraType(SPELL_AURA_MOUNTED)) {
                ...
                if (apply) {
                    if (target->HasAuraType(
                            SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED))
                        creatureEntry = 24906;
                    else
                        creatureEntry = 15665;
                } else
                    creatureEntry = target->GetAuraEffectsByType(
                            SPELL_AURA_MOUNTED).front()->GetMiscValue();
                ...
                if (CreatureInfo const* creatureInfo = ObjectMgr::GetCreatureTemplate(creatureEntry)) {
                    ...
                    uint32 display_id = sObjectMgr->ChooseDisplayId(team,
                            creatureInfo);
                    CreatureModelInfo const *minfo =
                            sObjectMgr->GetCreatureModelRandomGender(
                                    display_id);
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const *aurApp,
    ...
    if (!(apply)) {
        ...
        uint8 msg = plCaster->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest,
                GetSpellProto()->EffectItemType[m_effIndex], count,
                &noSpaceForCount);
        if (msg != EQUIP_ERR_OK) {
            count -= noSpaceForCount;
            plCaster->SendEquipError(msg, NULL, NULL,
                    GetSpellProto()->EffectItemType[m_effIndex]);
            if (count == 0)
                return;
        }

        Item *newitem = plCaster->StoreNewItem(dest,
                GetSpellProto()->EffectItemType[m_effIndex], true);
        if (!newitem) {
...
void AuraEffect::HandleBindSight(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleForceReaction(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK))
        return;
    ...
    player->GetReputationMgr().ApplyForceReaction(faction_id, faction_rank,
            apply);
...
void AuraEffect::HandleAuraEmpathy(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
    ...
    if (ci && ci->type == CREATURE_TYPE_BEAST)
        target->ApplyModUInt32Value(UNIT_DYNAMIC_FLAGS,
                UNIT_DYNFLAG_SPECIALINFO, apply);
}

void AuraEffect::HandleAuraModFaction(AuraApplication const *aurApp, uint8 mode,
        bool apply) const {
    if (!(mode & AURA_EFFECT_HANDLE_REAL))
        return;
...
void AuraEffect::HandleAuraModFakeInebriation(AuraApplication const * aurApp,
    ...
    if (apply) {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER) {
            int32 oldval = target->ToPlayer()->GetInt32Value(
                    PLAYER_FAKE_INEBRIATION);
            target->ToPlayer()->SetInt32Value(PLAYER_FAKE_INEBRIATION,
                    oldval + GetAmount());
        }
    } else {
