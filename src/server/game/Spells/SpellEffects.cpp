...
void Spell::SpellDamageSchoolDmg (SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        ...
        case SPELLFAMILY_GENERIC:
        {
            ...
            case 69041:
            {
                damage = uint32(1 + (0.25f * m_caster->GetTotalAttackPowerValue(BASE_ATTACK)) + (0.429f * m_caster->SpellBaseDamageBonus(SPELL_SCHOOL_MASK_FIRE)) + (m_caster->getLevel() * 2) + (m_caster->GetStat(STAT_INTELLECT) * 0.50193f));
            }
            }
            break;
        }
...
void Spell::EffectDummy (SpellEffIndex effIndex)
{
    ...
    case SPELLFAMILY_GENERIC:
    {
        ...
        {
            if (m_caster->GetTypeId() != TYPEID_PLAYER || !unitTarget->HasAura(51959) || !(m_caster->ToPlayer()->GetQuestStatus(12702) == QUEST_STATUS_INCOMPLETE || m_caster->ToPlayer()->GetQuestStatus(12532) == QUEST_STATUS_INCOMPLETE))
                return;
...
void Spell::EffectTriggerMissileSpell (SpellEffIndex effIndex)
{
    ...
    if (!spellInfo)
    {
        sLog->outError("EffectTriggerMissileSpell of spell %u (eff: %u): triggering unknown spell id %u", m_spellInfo->Id, effIndex, triggered_spell_id);
        return;
    }
...
void Spell::EffectHeal (SpellEffIndex /*effIndex*/)
{
    ...
    if (m_caster->HasAura(81208) /*Chakra: Serenity*/)
    {
        ...
        || m_spellInfo->Id == 32546 /*(Binding Heal)*/))
...
void Spell::SendLoot (uint64 guid, LootType loottype)
{
    ...
    if (gameObjTarget)
    {
        ...
        {
            sLog->outError("Possible hacking attempt: Player %s [guid: %u] tried to loot a gameobject [entry: %u id: %u] which is on respawn time without being in GM mode!", player->GetName(), player->GetGUIDLow(), gameObjTarget->GetEntry(), gameObjTarget->GetGUIDLow());
            return;
        }
...
void Spell::EffectSummonChangeItem (SpellEffIndex effIndex)
{
    ...
    else if (player->IsEquipmentPos(pos))
    {
        ...
        {
            if (msg == EQUIP_ERR_CANT_DO_RIGHT_NOW)
...
void Spell::EffectEnchantItemPerm (SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        {
            sLog->outCommand(p_caster->GetSession()->GetAccountId(), "GM %s (Account: %u) enchanting(perm): %s (Entry: %d) for player: %s (Account: %u)", p_caster->GetName(), p_caster->GetSession()->GetAccountId(), itemTarget->GetProto()->Name1, itemTarget->GetEntry(), item_owner->GetName(), item_owner->GetSession()->GetAccountId());
        }
...
void Spell::EffectEnchantItemPrismatic (SpellEffIndex effIndex)
{
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            sLog->outError("Spell::EffectEnchantItemPrismatic: attempt apply enchant spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u) but without ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not suppoted yet.", m_spellInfo->Id, SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC, ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET);
            return;
        }
    }
...
void Spell::EffectEnchantItemTmp (SpellEffIndex effIndex)
{
    ...
    if (item_owner != p_caster && p_caster->GetSession()->GetSecurity() > SEC_PLAYER && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
    {
        sLog->outCommand(p_caster->GetSession()->GetAccountId(), "GM %s (Account: %u) enchanting(temp): %s (Entry: %d) for player: %s (Account: %u)", p_caster->GetName(), p_caster->GetSession()->GetAccountId(), itemTarget->GetProto()->Name1, itemTarget->GetEntry(), item_owner->GetName(), item_owner->GetSession()->GetAccountId());
    }
...
void Spell::SpellDamageWeaponDmg (SpellEffIndex effIndex)
{
    ...
    case SPELLFAMILY_ROGUE:
    {
        ...
        if ((m_spellInfo->SpellFamilyFlags[1] & 0x40000) || (m_spellInfo->SpellFamilyFlags[0] & 0x6000000))
        {
            // Hemorrhage
