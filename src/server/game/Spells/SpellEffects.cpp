...
void Spell::EffectResurrectNew(SpellEffIndex effIndex)
{
    ...
    pTarget->setResurrectRequestData(m_caster->GetGUID(), m_caster->GetMapId(),
            m_caster->GetPositionX(), m_caster->GetPositionY(),
            m_caster->GetPositionZ(), health, mana);
...
void Spell::EffectInstaKill(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_spellInfo->Id == 18788 && unitTarget->GetTypeId() == TYPEID_UNIT)
    {
        ...
        switch (entry)
        {
            ...
            default:
                sLog->outError(
                        "EffectInstaKill: Unhandled creature entry (%u) case.",
                        entry);
                return;
        }
    ...
    m_caster->DealDamage(unitTarget, unitTarget->GetHealth(), NULL, NODAMAGE,
...
void Spell::EffectEnvirinmentalDMG(SpellEffIndex effIndex)
{
    ...
    damage = SpellMgr::CalculateSpellEffectAmount(m_spellInfo, effIndex,
            m_caster);

    m_caster->CalcAbsorbResist(m_caster, GetSpellSchoolMask(m_spellInfo),
            SPELL_DIRECT_DAMAGE, damage, &absorb, &resist, m_spellInfo);

    m_caster->SendSpellNonMeleeDamageLog(m_caster, m_spellInfo->Id, damage,
            GetSpellSchoolMask(m_spellInfo), absorb, resist, false, 0, false);
    if (m_caster->GetTypeId() == TYPEID_PLAYER) m_caster->ToPlayer()->EnvironmentalDamage(
            DAMAGE_FIRE, damage);
}
...
void Spell::SpellDamageSchoolDmg(SpellEffIndex effIndex)
{
            ...
            case SPELLFAMILY_GENERIC:
            {
                ...
                if (m_customAttr & SPELL_ATTR0_CU_SHARE_DAMAGE)
                {
                    ...
                    for (std::list <TargetInfo>::iterator ihit =
                        ...
                        if (ihit->effectMask & (1 << effIndex)) ++count;
                ...
                // better way to check unknown
                {
                    ...
                    case 39090:
                    case 39093:
                        if (!m_triggeredByAuraSpell) break;
                        if (unitTarget == m_caster)
                        {
                            ...
                            for (std::list <TargetInfo>::iterator ihit =
                                    m_UniqueTargetInfo.begin();
                                    ihit != m_UniqueTargetInfo.end(); ++ihit)
                                if (ihit->targetGUID != m_caster->GetGUID()) if (Player *target = ObjectAccessor::GetPlayer(*m_caster, ihit->targetGUID)) if (target->HasAura(
                                        m_triggeredByAuraSpell->Id)) ++count;
                            if (count)
                            {
                                uint32 spellId = 0;
                                switch (m_spellInfo->Id)
                                ...
                                m_caster->SetAuraStack(spellId, m_caster,
                                        count);
                            }
                        }

                        if (unitTarget->HasAura(m_triggeredByAuraSpell->Id)) damage =
                                0;
                        break;
                        // Consumption
                    case 28865:
                    ...
                    case 45072:
                    {
                        if (unitTarget->GetCreatureType() != CREATURE_TYPE_DEMON
                                && unitTarget->GetCreatureType()
                                        != CREATURE_TYPE_UNDEAD) return;
                        break;
                    }
                    ...
                    case 61547: // krystallus shatter ( Heroic )
                    {
                        ...
                        if (unitTarget->GetGUID() == m_caster->GetGUID()
                                || unitTarget->GetTypeId() != TYPEID_PLAYER) return;
                    ...
                    case 27831:
                    case 55638:
                        if (!unitTarget->HasAura(27825)) return;
                        break;
                        // Cataclysmic Bolt
                    case 38441:
                    {
                    ...
                    case 92631:
                    {
                        ...
                        if (unitTarget->GetUnitMovementFlags()
    ...
    {
      damage = uint32(m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * m_spellInfo->GetEffectMiscValue(0)/100); 
          ...
          m_caster->CastSpell(m_caster, trig_spell, true);
            ...
            case SPELLFAMILY_PRIEST:
            {
                switch (m_spellInfo->Id)
                {
                    ...
                    case 73510:
                        // Chakra: Chastise
                        if (m_caster->HasAura(14751)) m_caster->CastSpell(
                                m_caster, 81209, true);
                        break;
                    default:
                        break;
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                else if (m_spellInfo->SpellFamilyFlags [0] & 0x00000001)
                {
                    ...
                    if (AuraEffect const * aurEff = m_caster->GetDummyAuraEffect(SPELLFAMILY_DRUID, 1771, 0)) if (unitTarget->GetAuraEffect(
                            SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_DRUID,
                            0x00200000, 0, 0)) damage = int32(
            ...
            case SPELLFAMILY_ROGUE:
            {
                // Envenom
                if (m_caster->GetTypeId() == TYPEID_PLAYER
                        && (m_spellInfo->SpellFamilyFlags [1] & 0x8))
                {
                        ...
                        && m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    if (uint32 combo = ((Player*)m_caster)->GetComboPoints())
                    {
                        ...
                        // Eviscerate and Envenom Bonus Damage (item set effect)
                        if (m_caster->HasAura(37169)) damage += combo * 40;
                    }
                }
                break;
            }
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (m_spellInfo->SpellFamilyFlags [1] & 0x00040000)
                {
                    ...
                    float average = (m_caster->GetFloatValue(
                            UNIT_FIELD_MINDAMAGE)
                            + m_caster->GetFloatValue(UNIT_FIELD_MAXDAMAGE))
                            / float(
                                    m_caster->GetAttackTime(BASE_ATTACK)
                                            / 1000.0f);
                    int32 count = m_caster->CalculateSpellDamage(unitTarget,
                            m_spellInfo, EFFECT_2);
                    damage += count * int32(average * IN_MILLISECONDS)
                            / m_caster->GetAttackTime(BASE_ATTACK);
                    break;
                }
            ...
            case SPELLFAMILY_DEATHKNIGHT:
            {
                ...
                {
                    if (m_spellInfo->Id == 45462 || m_spellInfo->Id == 45477
                            || m_spellInfo->Id == 45524) m_caster->CastSpell(
                            unitTarget, 65142, true);
                }
                ...
                else if (m_spellInfo->SpellFamilyFlags [0] & 0x00040000)
                {
                    if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                            SPELLFAMILY_DEATHKNIGHT, 0, 0, 0x00000002,
                            m_caster->GetGUID()))
                    {
                        damage += m_damage / 2;
                        damage += int32(
            ...
            case SPELLFAMILY_MAGE:
            {
                ...
                if (m_spellInfo->Id == 71757) if (unitTarget->GetTypeId()
                        != TYPEID_UNIT
...
{
    if (!unitTarget && !gameObjTarget && !itemTarget) return;
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 12868:
                {
                    ...
                    switch (m_spellInfo->Id)
                    {
                        ...
                        default:
                            sLog->outError(
                                    "Spell::EffectDummy: Spell %u not handled in DW",
                                    m_spellInfo->Id);
                            return;
                    };
                ...
                case 13567: // Dummy Trigger
                {
                    ...
                    if (!m_triggeredByAuraSpell || !unitTarget) return;

                    switch (m_triggeredByAuraSpell->Id)
                    {
                        case 26467: // Persistent Shield
                            m_caster->CastCustomSpell(unitTarget, 26470,
                                    &damage, NULL, NULL, true);
                            break;
                        default:
                            sLog->outError(
                                    "EffectDummy: Non-handled case for spell 13567 for triggered aura %u",
                ...
                {
                    if (!unitTarget || !m_originalCaster) return;

                    if (m_originalCaster->GetTypeId() == TYPEID_PLAYER)
                    {
                        WorldPacket data(SMSG_SPIRIT_HEALER_CONFIRM, 8);
                        data << uint64(unitTarget->GetGUID());
                        m_originalCaster->ToPlayer()->GetSession()->SendPacket(
                                &data);
                    }
                    return;
                }
                case 20577: // Cannibalize
                    if (unitTarget) m_caster->CastSpell(m_caster, 20578, false,
                            NULL);
                    return;
                case 23019: // Crystal Prison Dummy DND
                {
                    ...
                    m_caster->SummonGameObject(
                            179644,
                            creatureTarget->GetPositionX(),
                            creatureTarget->GetPositionY(),
                            creatureTarget->GetPositionZ(),
                            ...
                            0,
                            uint32(
                                    creatureTarget->GetRespawnTime()
                                            - time(NULL)));
                    sLog->outDebug(
                            LOG_FILTER_SPELLS_AURAS,
                            "SummonGameObject at SpellEfects.cpp EffectDummy for Spell 23019");
                ...
                case 23453: // Gnomish Transporter - Ultrasafe Transporter: Gadgetzan
                    ...
                    // Gadgetzan Transporter Failure - failure
                ...
                case 26074: // Holiday Cheer
                    ...
                    // Polarity Shift
                case 28089:
                    if (unitTarget) unitTarget->CastSpell(unitTarget,
                            roll_chance_i(50) ? 28059 : 28084, true, NULL, NULL,
                            m_caster->GetGUID());
                    break;
                    // Polarity Shift
                case 39096:
                    if (unitTarget) unitTarget->CastSpell(unitTarget,
                            roll_chance_i(50) ? 39088 : 39091, true, NULL, NULL,
                            m_caster->GetGUID());
                    break;
                case 29200: // Purify Helboar Meat
                {
                    ...
                    uint32 spell_id = roll_chance_i(50) ? 29277 // Summon Purified Helboar Meat
                ...
                case 29858: // Soulshatter
                    if (unitTarget && unitTarget->CanHaveThreatList()
                            && unitTarget->getThreatManager().getThreat(
                                    m_caster) > 0.0f) m_caster->CastSpell(
                            unitTarget, 32835, true);
                    return;
                ...
                case 37674: // Chaos Blast
                {
                    ...
                    m_caster->CastCustomSpell(unitTarget, 37675, &basepoints0,
                            NULL, NULL, true);
                    return;
                }
                ...
                case 42784:
                {
                    ...
                    for (std::list <TargetInfo>::iterator ihit =
                        ...
                        {
                            if (Unit* casttarget = Unit::GetUnit((*unitTarget), ihit->targetGUID)) m_caster->DealDamage(
                                    casttarget, damage, NULL,
                                    SPELL_DIRECT_DAMAGE,
                                    SPELL_SCHOOL_MASK_ARCANE, spellInfo, false);
                        }
                ...
                case 58984:                               // Shadowmeld
                {
                    ...
                    m_caster->AttackStop();
                    m_caster->CombatStop();
                    if (m_caster->ToPlayer()) m_caster->ToPlayer()->SendAttackSwingCancelAttack();
                    return;
                }
                ...
                case 43723:
                {
                    ...
                    if (player
                            && player->GetQuestStatus(11379)
                                    == QUEST_STATUS_INCOMPLETE)
                    {
                        Creature *creature = player->FindNearestCreature(19973,
                                10, false);
                        if (!creature)
                        {
                ...
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER) return;

                    // If target has spectral exhaustion or spectral realm aura return
                    if (unitTarget->HasAura(44867)
                            || unitTarget->HasAura(46021)) return;

                    // Cast the spectral realm effect spell, visual spell and spectral blast rift summoning
                    unitTarget->CastSpell(unitTarget, 44866, true, NULL, NULL,
                            m_caster->GetGUID());
                    unitTarget->CastSpell(unitTarget, 46648, true, NULL, NULL,
                            m_caster->GetGUID());
                ...
                {
                    if (!unitTarget && unitTarget->GetEntry() != 26452
                            && unitTarget->HealthAbovePct(95)) return;

                    m_caster->DealDamage(unitTarget,
                ...
                case 48046: // Use Camera
                {
                    ...
                    m_caster->CastSpell(m_caster, 48047, true, m_CastItem, NULL,
                            unitTarget->GetGUID());
                    return;
                }
                ...
                case 49357: // Brewfest Mount Transformation
                    ...
                    {
                        if (m_caster->ToPlayer()->GetSpeedRate(MOVE_RUN)
                                >= 2.0f)
                        // 100% Kodo
                ...
                case 51582: //Rocket Boots Engaged (Rocket Boots Xtreme and Rocket Boots Xtreme Lite)
                {
                    ...
                    if (Battleground* bg = m_caster->ToPlayer()->GetBattleground()) bg->EventPlayerDroppedFlag(
                            m_caster->ToPlayer());
                ...
                case 62324: // Throw Passenger
                {
                    ...
                    {
                        if (Vehicle *vehicle = m_caster->GetVehicleKit()) if (Unit *passenger = vehicle->GetPassenger(damage - 1))
                        {
                            std::list <Unit*> unitList;
                            // use 99 because it is 3d search
                            SearchAreaTarget(unitList, 99, PUSH_DST_CENTER,
                                    SPELL_TARGETS_ENTRY, 33114);
                            float minDist = 99 * 99;
                            Vehicle *target = NULL;
                            for (std::list <Unit*>::iterator itr =
                                    unitList.begin(); itr != unitList.end();
                                    ++itr)
                            {
                                if (Vehicle *seat = (*itr)->GetVehicleKit()) if (!seat->GetPassenger(
                                        0)) if (Unit *device = seat->GetPassenger(2)) if (!device->GetCurrentSpell(
                                        CURRENT_CHANNELED_SPELL))
                                {
                                    float dist = (*itr)->GetExactDistSq(
                                            &m_targets.m_dstPos);
                                    if (dist < minDist)
                                    {
                                        minDist = dist;
                            ...
                            if (target
                                    && target->GetBase()->IsWithinDist2d(
                                            &m_targets.m_dstPos,
                                            GetSpellRadius(m_spellInfo,
                                                    effIndex, false) * 2)) // now we use *2 because the location of the seat is not correct
                            ...
                            else
                            {
                                ...
                                passenger->GetMotionMaster()->MoveJump(x, y, z,
                                        m_targets.GetSpeedXY(),
                                        m_targets.GetSpeedZ());
                            }
                        }
                    }
                ...
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER
                            && !m_caster->isInCombat()) m_caster->ToPlayer()->toggleWorgenForm();
                    return;
                }
                case 53808: // Pygmy Oil
                {
                    Aura *pAura = m_caster->GetAura(53806);
                    if (pAura) pAura->RefreshDuration();
                    else
                    {
                        pAura = m_caster->GetAura(53805);
                        if (!pAura || pAura->GetStackAmount() < 5
                                || !roll_chance_i(50)) m_caster->CastSpell(
                                m_caster, 53805, true);
                        else
                        {
                            pAura->Remove();
                ...
                case 54577: // U.D.E.D.
                {
                    if (unitTarget->GetEntry() != 29402) return;

                    m_caster->SummonGameObject(192693,
                            unitTarget->GetPositionX(),
                            unitTarget->GetPositionY(),
                            unitTarget->GetPositionZ(),
                            unitTarget->GetOrientation(), 0, 0, 0, 0, 100);
                    for (uint8 i = 0; i < 4; ++i)
                        m_caster->SummonGameObject(
                                191567,
                                float(
                ...
                case 51961: // Captured Chicken Cover - Quest 12702 & 12532
                {
                    if (m_caster->GetTypeId() != TYPEID_PLAYER
                            ...
                            || !(m_caster->ToPlayer()->GetQuestStatus(12702)
                                    == QUEST_STATUS_INCOMPLETE
                                    || m_caster->ToPlayer()->GetQuestStatus(
                                            12532) == QUEST_STATUS_INCOMPLETE)) return;
                ...
                case 69228: // Throw Torch
                {
                    ...
                    {
                        case 36727:
                            KillCredit = unitTarget->GetEntry();
                            break;
                        case 37155:
                            KillCredit = unitTarget->GetEntry();
                            break;
                ...
                {
                    if (m_caster->GetTypeId() != TYPEID_PLAYER) return;
                    if (unitTarget->GetEntry() != 42224) return;

                    m_caster->Kill(unitTarget, false);
                    m_caster->ToPlayer()->KilledMonsterCredit(
                            unitTarget->GetEntry(), unitTarget->GetGUID());
                }
        ...
        case SPELLFAMILY_HUNTER:
            ...
            if (m_spellInfo->SpellFamilyFlags [1] & 0x1) m_caster->CastSpell(
                    m_caster, 77443, true);
            if (m_spellInfo->SpellFamilyFlags [2] & 0x20) m_caster->CastSpell(
                    m_caster, 51755, true);
            break;
        case SPELLFAMILY_PRIEST:
        {
                ...
                case 21562: // Power Word : Fortitude
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        if (Continue == true) m_caster->CastSpell(unitTarget,
                                79105, true); // Power Word : Fortitude (Raid)
                        else m_caster->CastSpell(unitTarget, 79104, true); // Power Word : Fortitude (Caster)
                    }
                    break;
                }
            }
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags [0]
                    & SPELLFAMILYFLAG1_MAGE_CONEOFCOLD)
            {
                if (m_caster->HasAura(11190)) // Improved Cone of Cold Rank 1
                {
                    m_caster->CastCustomSpell(unitTarget, 83301, &bp, NULL,
                            NULL, true, 0);
                }
                if (m_caster->HasAura(12489)) // Improved Cone of Cold Rank 2
                {
                    m_caster->CastCustomSpell(unitTarget, 83302, &bp, NULL,
                            NULL, true, 0);
                }
            }
            switch (m_spellInfo->Id)
            {
                case 1459: // Arcane Brilliance
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        if (Continue == true) m_caster->CastSpell(unitTarget,
                                79058, true); // Arcane Brilliance (For all)
                        else m_caster->CastSpell(unitTarget, 79057, true); // Arcane Brilliance (Only for caster)
                    }
                    break;
                }
                case 42955: // Conjure Refreshment
                {
                    if (m_caster->getLevel() > 33 && m_caster->getLevel() < 44) m_caster->CastSpell(
                            m_caster, 92739, true);
                    if (m_caster->getLevel() > 43 && m_caster->getLevel() < 54) m_caster->CastSpell(
                            m_caster, 92799, true);
                    if (m_caster->getLevel() > 53 && m_caster->getLevel() < 65) m_caster->CastSpell(
                            m_caster, 92802, true);
                    if (m_caster->getLevel() > 64 && m_caster->getLevel() < 74) m_caster->CastSpell(
                            m_caster, 92805, true);
                    if (m_caster->getLevel() > 73 && m_caster->getLevel() < 80) m_caster->CastSpell(
                            m_caster, 74625, true);
                    if (m_caster->getLevel() > 79 && m_caster->getLevel() < 85) m_caster->CastSpell(
                            m_caster, 92822, true);
                    if (m_caster->getLevel() == 85) m_caster->CastSpell(
                            m_caster, 92727, true);
                    break;
                }
                case 82731: // Flame Orb
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER) m_caster->CastSpell(
                            m_caster, 84765, true); // Summon Flame Orb
                    break;
                }
                case 43987: // Ritual of Refreshment
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        if (m_caster->getLevel() > 75
                                && m_caster->getLevel() < 80) m_caster->CastSpell(
                                m_caster, 74650, true);
                        if (m_caster->getLevel() > 80
                                && m_caster->getLevel() < 85) m_caster->CastSpell(
                                m_caster, 92824, true);
                        if (m_caster->getLevel() == 85) m_caster->CastSpell(
                                m_caster, 92827, true);
                    }
        ...
        case SPELLFAMILY_WARRIOR:
            // Charge
            if (m_spellInfo->SpellFamilyFlags & SPELLFAMILYFLAG_WARRIOR_CHARGE
                    && m_spellInfo->SpellVisual [0] == 867)
            {
                int32 chargeBasePoints0 = damage;
                m_caster->CastCustomSpell(m_caster, 34846, &chargeBasePoints0,
                        NULL, NULL, true);
            ...
            //Slam
            if (m_spellInfo->SpellFamilyFlags [0] & SPELLFAMILYFLAG_WARRIOR_SLAM
                    && m_spellInfo->SpellIconID == 559)
            {
                int32 bp0 = damage;
                m_caster->CastCustomSpell(unitTarget, 50782, &bp0, NULL, NULL,
                        true, 0);
                return;
            }
            // Concussion Blow
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 19740: // Blessing of Might
                {
                    if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    {
                        ...
                        else m_caster->CastSpell(unitTarget, 79101, true); // Blessing of Might (Caster)
                    }
                    break;
                }
                case 31789: // Righteous Defense (step 1)
                {
                    ...
                    for (Unit::AttackerSet::iterator aItr = attackers.begin();
                            aItr != attackers.end();)
                        if (!(*aItr)->canAttack(m_caster)) attackers.erase(
                                aItr++);
                        else ++aItr;

                    // selected from list 3
                    uint32 maxTargets = std::min < uint32
                            > (3, attackers.size());
        ...
        case SPELLFAMILY_SHAMAN:
                    ...
                    && m_spellInfo->SpellIconID == 1673)
            {
                ...
                if (unitTarget) m_caster->CastCustomSpell(unitTarget, 52025,
                        NULL, &bp1, NULL, true, NULL, NULL,
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            if (m_spellInfo->SpellFamilyFlags [1]
                    & SPELLFAMILYFLAG1_DK_HUNGERING_COLD)
            {
                m_caster->CastCustomSpell(m_caster, 51209, &bp, NULL, NULL,
                        true);
            }
            // Chains of Ice
            if (m_spellInfo->SpellFamilyFlags [0]
                    & SPELLFAMILYFLAG_DK_CHAINS_OF_ICE)
            {
            ...
            if (m_spellInfo->SpellFamilyFlags [0] & SPELLFAMILYFLAG_DK_DEATH_COIL)
            {
                if (m_caster->IsFriendlyTo(unitTarget))
                {
                    ...
                    m_caster->CastCustomSpell(unitTarget, 47633, &bp, NULL,
                            NULL, true);
                }
                else
                {
                    int32 bp = 985 + damage;
                    m_caster->CastCustomSpell(unitTarget, 47632, &bp, NULL,
                            NULL, true);
                }
                return;
            }
            switch (m_spellInfo->Id)
            {
                ...
                case 46584: // Raise Dead
                {
                    ...
                    if (m_targets.HasDst()) targets.setDst(m_targets.m_dstPos);
                    else
                    {
    ...
    if (spell_id)
    {
        ...
        Spell* spell = new Spell(m_caster, spellInfo, triggered,
                m_originalCasterGUID, true);
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
            "Spell ScriptStart spellid %u in EffectDummy(%u)", m_spellInfo->Id,
            effIndex);
    m_caster->GetMap()->ScriptsStart(sSpellScripts,
            uint32(m_spellInfo->Id | (effIndex << 24)), m_caster, unitTarget);

    // Script based implementation. Must be used only for not good for implementation in core spell effects
    // So called only for not proccessed cases
    if (gameObjTarget)
    sScriptMgr->OnDummyEffect(m_caster, m_spellInfo->Id, effIndex,
            gameObjTarget);
    else if (unitTarget && unitTarget->GetTypeId() == TYPEID_UNIT)
    sScriptMgr->OnDummyEffect(m_caster, m_spellInfo->Id, effIndex,
            unitTarget->ToCreature());
    else if (itemTarget)
...
void Spell::EffectTriggerSpellWithValue(SpellEffIndex effIndex)
{
    ...
    {
        sLog->outError(
                "EffectTriggerSpellWithValue of spell %u: triggering unknown spell id %i",
    ...
    caster->CastCustomSpell(unitTarget, triggered_spell_id, &bp, &bp, &bp,
            true);
}
...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    ...
    if (damage)
    {
        switch (m_spellInfo->Id)
        {
            ...
            case 52349: // Overtake
                unitTarget->CastCustomSpell(unitTarget, spellInfo->Id, &damage,
                        NULL, NULL, true, NULL, NULL, m_originalCasterGUID);
                return;
            case 72378: // Blood Nova
            case 73058: // Blood Nova
                spellInfo = sSpellMgr->GetSpellForDifficultyFromSpell(spellInfo,
                        m_caster);
                break;
        }
    }
    ...
    {
        case 62056:
        case 63985: // Stone Grip Forcecast (10m, 25m)
    ...
    caster->CastSpell(unitTarget, spellInfo, true, NULL, NULL,
            m_originalCasterGUID);
}
...
void Spell::EffectForceCastWithValue(SpellEffIndex effIndex)
{
    ...
    {
        sLog->outError(
                "EffectForceCastWithValue of spell %u: triggering unknown spell id %i",
                m_spellInfo->Id, triggered_spell_id);
        return;
    }
    int32 bp = damage;
    ...
    caster->CastCustomSpell(unitTarget, spellInfo->Id, &bp, &bp, &bp, true,
            NULL, NULL, m_originalCasterGUID);
}
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex)
{
    ...
    if (!unitTarget)
    {
        ...
        sLog->outError(
                "Spell::EffectTriggerSpell (Spell: %u): Unsupported non-unit case!",
                m_spellInfo->Id);
        return;
    }
    ...
    switch (triggered_spell_id)
    {
        ...
        case 58832:
        {
            ...
            if (m_caster->HasAura(63093)) m_caster->CastSpell(m_caster, 65047,
                    true); // Mirror Image
        ...
        case 35729:
        {
            ...
            for (Unit::AuraApplicationMap::iterator iter = Auras.begin();
                    iter != Auras.end();)
            {
                // remove all harmful spells on you...
                SpellEntry const* spell =
                        iter->second->GetBase()->GetSpellProto();
                if ((spell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC // only affect magic spells
                || ((1 << spell->Dispel) & dispelMask))
                // ignore positive and passive auras
                        && !iter->second->IsPositive()
                        && !iter->second->GetBase()->IsPassive())
                {
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER
            && m_spellInfo->CategoryRecoveryTime
            && spellInfo->CategoryRecoveryTime
            && m_spellInfo->Category == spellInfo->Category) m_caster->ToPlayer()->RemoveSpellCooldown(
            spellInfo->Id);
    ...
    caster->CastSpell(unitTarget, spellInfo, true, 0, 0,
            (originalCaster ? originalCaster->GetGUID() : 0));
}
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex)
{
    ...
    {
        sLog->outError(
                "EffectTriggerMissileSpell of spell %u (eff: %u): triggering unknown spell id %u",
...
void Spell::EffectJump(SpellEffIndex effIndex)
{
    ...
    float x, y, z;
    if (m_targets.getUnitTarget()) m_targets.getUnitTarget()->GetContactPoint(
            m_caster, x, y, z, CONTACT_DISTANCE);
    else if (m_targets.getGOTarget()) m_targets.getGOTarget()->GetContactPoint(
            m_caster, x, y, z, CONTACT_DISTANCE);
    else
    {
        sLog->outError(
                "Spell::EffectJump - unsupported target mode for spell ID %u",
                m_spellInfo->Id);
        return;
    }
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
    ...
    {
        sLog->outError(
                "Spell::EffectJumpDest - unsupported target mode for spell ID %u",
                m_spellInfo->Id);
        return;
    }
...
void Spell::CalculateJumpSpeeds(uint8 i, float dist, float & speedXY,
        float & speedZ)
{
    if (m_spellInfo->EffectMiscValue [i]) speedZ = float(
            m_spellInfo->EffectMiscValue [i]) / 10;
    else if (m_spellInfo->EffectMiscValueB [i]) speedZ = float(
            m_spellInfo->EffectMiscValueB [i]) / 10;
    else speedZ = 10.0f;
...
{
    if (!unitTarget || unitTarget->isInFlight()) return;
    ...
    switch (m_spellInfo->Id)
    {
        case 36563: // Shadowstep
            if (Player * plr = unitTarget->ToPlayer())
            {
                if (Unit * target = plr->GetSelectedUnit())
                {
                    ...
                    m_targets.setDst(pos.GetPositionX(), pos.GetPositionY(),
                            pos.GetPositionZ(), target->GetOrientation());
                }
            }
            break;
        ...
        case 60320: // Scroll of Recall II
            if (!uiMaxSafeLevel) uiMaxSafeLevel = 70;
        case 60321: // Scroll of Recal III
            if (!uiMaxSafeLevel) uiMaxSafeLevel = 80;
        ...
        case 66550: // teleports outside (Isle of Conquest)
            ...
            {
                if (pTarget->GetTeamId() == TEAM_ALLIANCE) m_targets.setDst(
                        442.24f, -835.25f, 44.30f, 0.06f, 628);
                else m_targets.setDst(1120.43f, -762.11f, 47.92f, 2.94f, 628);
            }
            break;
        case 66551: // teleports inside (Isle of Conquest)
            ...
            {
                if (pTarget->GetTeamId() == TEAM_ALLIANCE) m_targets.setDst(
                        389.57f, -832.38f, 48.65f, 3.00f, 628);
                else m_targets.setDst(1174.85f, -763.24f, 48.72f, 6.26f, 628);
            }
            break;
    }
    ...
    {
        sLog->outError(
                "Spell::EffectTeleportUnits - does not have destination for spell ID %u\n",
                m_spellInfo->Id);
        return;
    }
    ...
    if (!orientation && m_targets.getUnitTarget()) orientation =
            m_targets.getUnitTarget()->GetOrientation();
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
            "Spell::EffectTeleportUnits - teleport unit to %u %f %f %f %f\n",
            mapid, x, y, z, orientation);

    if (mapid == unitTarget->GetMapId()) unitTarget->NearTeleportTo(x, y, z,
            orientation, unitTarget == m_caster);
    else if (unitTarget->GetTypeId() == TYPEID_PLAYER) unitTarget->ToPlayer()->TeleportTo(
            mapid, x, y, z, orientation,
            unitTarget == m_caster ? TELE_TO_SPELL : 0);
            ...
            if (roll_chance_i(50)) // 50% success
            {
                ...
                switch (rand_eff)
                {
                    ...
                    case 5:
                        // Transform
                    {
                        if (m_caster->ToPlayer()->GetTeam() == ALLIANCE) m_caster->CastSpell(
                                m_caster, 36897, true);
                        else m_caster->CastSpell(m_caster, 36899, true);
                        break;
                    }
                    case 6:
                        // chicken
...
void Spell::EffectApplyAura(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            {
                if (m_caster->HasAura(81208 /*Chakra: Serenity*/)) m_caster->RemoveAura(
                        81208);
                else if (m_caster->HasAura(81206 /*Chakra: Sanctuary*/)) m_caster->RemoveAura(
                        81206);
                else if (m_caster->HasAura(81209 /*Chakra: Chastise*/)) m_caster->RemoveAura(
                        81209);
            }
            ...
            if (m_caster->HasAura(14751 /*Chakra*/)
                    && m_spellInfo->Id == 41635 /*Prayer of Mending */) m_caster->CastSpell(
                    m_caster, 81206, true); // Chakra: Sanctuary
...
{
    if (m_spellInfo->EffectMiscValue [effIndex] < 0
            || m_spellInfo->EffectMiscValue [effIndex] >= int8(MAX_POWERS)) return;

    Powers powerType = Powers(m_spellInfo->EffectMiscValue [effIndex]);

    if (!unitTarget || !unitTarget->isAlive()
            || unitTarget->getPowerType() != powerType || damage < 0) return;

    // add spell damage bonus
    damage = m_caster->SpellDamageBonus(unitTarget, m_spellInfo, effIndex,
            uint32(damage), SPELL_DIRECT_DAMAGE);
    ...
    {
        gainMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(
                m_spellInfo, effIndex, m_originalCaster, this);

        int32 gain = int32(newDamage * gainMultiplier);

        m_caster->EnergizeBySpell(m_caster, m_spellInfo->Id, gain, powerType);
    }
    ExecuteLogEffectTakeTargetPower(effIndex, unitTarget, powerType, newDamage,
            gainMultiplier);
}
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
            ...
            m_spellInfo->EffectMiscValue [effIndex], m_spellInfo->Id);

    Object *pTarget;
    if (focusObject) pTarget = focusObject;
    else if (unitTarget) pTarget = unitTarget;
    else if (gameObjTarget) pTarget = gameObjTarget;
    else pTarget = NULL;

    if (unitTarget)
    {
        if (ZoneScript* zoneScript = unitTarget->GetZoneScript()) zoneScript->ProcessEvent(
                unitTarget, m_spellInfo->EffectMiscValue [effIndex]);
    }

    m_caster->GetMap()->ScriptsStart(sEventScripts,
            m_spellInfo->EffectMiscValue [effIndex], m_caster, pTarget);
}
...
void Spell::SpellDamageHeal(SpellEffIndex effIndex)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        {
            if (Player* player = m_caster->ToPlayer()) if (player->HasSkill(
                        ...
                        m_caster))
        {
            ...
            {
                sLog->outError(
                        "Target(GUID:" UI64FMTD ") has aurastate AURA_STATE_SWIFTMEND but no matching aura.",
                        unitTarget->GetGUID());
                return;
            }
        ...
        // Glyph of Nourish
        else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DRUID
                && m_spellInfo->SpellFamilyFlags [1] & 0x2000000)
        {
            addhealth = caster->SpellHealingBonus(unitTarget, m_spellInfo,
                    effIndex, addhealth, HEAL);

            if (AuraEffect const* aurEff = m_caster->GetAuraEffect(62971, 0))
            {
                Unit::AuraEffectList const& Periodic =
                        unitTarget->GetAuraEffectsByType(
                                SPELL_AURA_PERIODIC_HEAL);
                for (Unit::AuraEffectList::const_iterator i = Periodic.begin();
                        i != Periodic.end(); ++i)
                {
                    if (m_caster->GetGUID() == (*i)->GetCasterGUID()) addhealth +=
                            addhealth * aurEff->GetAmount() / 100;
                }
            }
        }
        // Lifebloom - final heal coef multiplied by original DoT stack
        else if (m_spellInfo->Id == 33778) addhealth =
                caster->SpellHealingBonus(unitTarget, m_spellInfo, effIndex,
                        addhealth, HEAL, m_spellValue->EffectBasePoints [1]);
        // Riptide - increase healing done by Chain Heal
        else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN
                && m_spellInfo->SpellFamilyFlags [0] & 0x100)
        {
            addhealth = caster->SpellHealingBonus(unitTarget, m_spellInfo,
                    effIndex, addhealth, HEAL);
        ...
        else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT
...
void Spell::EffectHealPct(SpellEffIndex effIndex)
{
    ...
    // Rune Tap - Party
    if (m_spellInfo->Id == 59754 && unitTarget == m_caster) return;

    if (m_spellInfo->Id == 34428 && m_originalCaster->HasAura(82368)) damage =
            5;

    m_healing += m_originalCaster->SpellHealingBonus(unitTarget, m_spellInfo,
            effIndex, unitTarget->CountPctFromMaxHealth(damage), HEAL);
}
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    float healMultiplier = SpellMgr::CalculateSpellEffectValueMultiplier(
            m_spellInfo, effIndex, m_originalCaster, this);

    m_damage += damage;
    // get max possible damage, don't count overkill for heal
    uint32 healthGain = uint32(
            -unitTarget->GetHealthGain(-damage) * healMultiplier);

    if (m_caster->isAlive())
    {
        healthGain = m_caster->SpellHealingBonus(m_caster, m_spellInfo,
                effIndex, healthGain, HEAL);
...
void Spell::DoCreateItem(uint32 /*i*/, uint32 itemtype)
{
    ...
    if (num_to_add > pProto->GetMaxStackSize()) num_to_add =
    ...
    uint32 no_space = 0;
    uint8 msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, newitemid,
            num_to_add, &no_space);
    if (msg != EQUIP_ERR_OK)
    {
        // convert to possible store amount
        if (msg == EQUIP_ERR_INVENTORY_FULL
                || msg == EQUIP_ERR_CANT_CARRY_MORE_OF_THIS) num_to_add -=
                no_space;
        else
        {
    ...
    if (num_to_add)
    {
        ...
        Item* pItem = player->StoreNewItem(dest, newitemid, true,
                Item::GenerateItemRandomPropertyId(newitemid));
        ...
        if (pItem->GetProto()->Class != ITEM_CLASS_CONSUMABLE
                ...
                && newitemid != 6265 && newitemid != 6948) pItem->SetUInt32Value(
                ITEM_FIELD_CREATOR, player->GetGUIDLow());

        // send info to the client
        if (pItem) player->SendNewItem(pItem, num_to_add, true, bgType == 0);

        // we succeeded in creating at least one item, so a levelup is possible
        if (bgType == 0) player->UpdateCraftSkill(m_spellInfo->Id);
    }
...
{
    if (m_spellInfo->EffectItemType [effIndex] == 6948
            && unitTarget->ToPlayer()->GetItemByEntry(6948)) //dont create a new hearthstone on homebind if player already has
    return;
    DoCreateItem(effIndex, m_spellInfo->EffectItemType [effIndex]);
    ExecuteLogEffectCreateItem(effIndex,
            m_spellInfo->EffectItemType [effIndex]);
}
...
void Spell::EffectCreateItem2(SpellEffIndex effIndex)
{
    ...
    if (item_id) DoCreateItem(effIndex, item_id);
    ...
    if (IsLootCraftingSpell(m_spellInfo))
    {
        ...
        else player->AutoStoreLoot(m_spellInfo->Id, LootTemplates_Spell); // create some random items
    }
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
    if (!m_spellAura)
    {
        float radius = GetSpellRadiusForFriend(
                sSpellRadiusStore.LookupEntry(
                        m_spellInfo->EffectRadiusIndex [effIndex]));
        if (Player* modOwner = m_originalCaster->GetSpellModOwner()) modOwner->ApplySpellMod(
                m_spellInfo->Id, SPELLMOD_RADIUS, radius);

        Unit *caster =
                m_caster->GetEntry() == WORLD_TRIGGER ?
                        m_originalCaster : m_caster;
        // Caster not in world, might be spell triggered from aura removal
        if (!caster->IsInWorld()) return;
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    {
        case 2687: // Bloodrage
            if (m_caster->HasAura(70844)) m_caster->CastSpell(m_caster, 70845,
                    true);
            break;
        ...
        {
            if (Player* player = m_caster->ToPlayer()) if (player->HasSkill(
                    SKILL_ENGINEERING)) AddPctN(damage, 25);
            break;
        }
        default:
            break;
    ...
    if (level_diff > 0) damage -= level_multiplier * level_diff;

    if (damage < 0) return;

    if (unitTarget->GetMaxPower(power) == 0) return;
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        Unit::AuraApplicationMap& Auras = unitTarget->GetAppliedAuras();
        for (Unit::AuraApplicationMap::iterator itr = Auras.begin();
                itr != Auras.end(); ++itr)
        {
            uint32 spell_id = itr->second->GetBase()->GetId();
            if (!guardianFound) if (sSpellMgr->IsSpellMemberOfSpellGroup(
                    spell_id, SPELL_GROUP_ELIXIR_GUARDIAN)) guardianFound =
                    true;
            if (!battleFound) if (sSpellMgr->IsSpellMemberOfSpellGroup(spell_id,
                    SPELL_GROUP_ELIXIR_BATTLE)) battleFound = true;
            if (battleFound && guardianFound) break;
        }

        // get all available elixirs by mask and spell level
        std::set <uint32> avalibleElixirs;
        if (!guardianFound)
        sSpellMgr->GetSetOfSpellsInSpellGroup(SPELL_GROUP_ELIXIR_GUARDIAN,
                avalibleElixirs);
        if (!battleFound)
        sSpellMgr->GetSetOfSpellsInSpellGroup(SPELL_GROUP_ELIXIR_BATTLE,
                avalibleElixirs);
        for (std::set <uint32>::iterator itr = avalibleElixirs.begin();
                itr != avalibleElixirs.end();)
        {
            SpellEntry const *spellInfo = sSpellStore.LookupEntry(*itr);
            if (spellInfo->spellLevel < m_spellInfo->spellLevel
                    || spellInfo->spellLevel > unitTarget->getLevel()) avalibleElixirs.erase(
                    itr++);
            else if (sSpellMgr->IsSpellMemberOfSpellGroup(*itr,
                    SPELL_GROUP_ELIXIR_SHATTRATH)) avalibleElixirs.erase(itr++);
            else if (sSpellMgr->IsSpellMemberOfSpellGroup(*itr,
                    SPELL_GROUP_ELIXIR_UNSTABLE)) avalibleElixirs.erase(itr++);
            else ++itr;
        }
    ...
    if (gameObjTarget)
    {
        ...
        if (!gameObjTarget->isSpawned() && !player->isGameMaster())
        {
            sLog->outError(
                    ...
                    gameObjTarget->GetEntry(), gameObjTarget->GetGUIDLow());
            return;
        }

        if (sScriptMgr->OnGossipHello(player, gameObjTarget)) return;
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_BUTTON:
                ...
                player->GetMap()->ScriptsStart(sGameObjectScripts,
                        gameObjTarget->GetDBTableGUIDLow(), player,
                        gameObjTarget);
                return;
            ...
            case GAMEOBJECT_TYPE_SPELL_FOCUS:
                ...
                if (uint32 trapEntry = gameObjTarget->GetGOInfo()->spellFocus.linkedTrapId) gameObjTarget->TriggeringLinkedGameObject(
                        trapEntry, m_caster);
                return;
            ...
            case GAMEOBJECT_TYPE_CHEST:
                ...
                {
                    sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
                            "Chest ScriptStart id %u for GO %u",
                            gameObjTarget->GetGOInfo()->chest.eventId,
                            gameObjTarget->GetDBTableGUIDLow());
                    player->GetMap()->ScriptsStart(sEventScripts,
                            gameObjTarget->GetGOInfo()->chest.eventId, player,
                            gameObjTarget);
                }

                // triggering linked GO
                if (uint32 trapEntry = gameObjTarget->GetGOInfo()->chest.linkedTrapId) gameObjTarget->TriggeringLinkedGameObject(
                        trapEntry, m_caster);
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if (gameObjTarget)
    {
        ...
        else if (goInfo->type == GAMEOBJECT_TYPE_FLAGSTAND)
        {
            ...
            if (Battleground *bg = player->GetBattleground())
            {
                if (bg->GetTypeID(true) == BATTLEGROUND_EY) bg->EventPlayerClickedOnFlag(
                        player, gameObjTarget);
                return;
            }
        }
        else if (m_spellInfo->Id == 1842
                && gameObjTarget->GetGOInfo()->type == GAMEOBJECT_TYPE_TRAP
                && gameObjTarget->GetOwner())
        {
        ...
        else if (gameObjTarget->GetGOInfo()->type == GAMEOBJECT_TYPE_GOOBER
                && sOutdoorPvPMgr->HandleOpenGo(player,
                        gameObjTarget->GetGUID())) return;
    ...
    SpellCastResult res = CanOpenLock(effIndex, lockId, skillId, reqSkillValue,
            skillValue);
    ...
    if (gameObjTarget) SendLoot(guid, LOOT_SKINNING);
    else itemTarget->SetFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_UNLOCKED);
        ...
        if (uint32 pureSkillValue = player->GetPureSkillValue(skillId))
        {
            if (gameObjTarget)
            {
                ...
                if (!gameObjTarget->IsInSkillupList(player->GetGUIDLow())
                        && player->UpdateGatherSkill(skillId, pureSkillValue,
                                reqSkillValue)) gameObjTarget->AddToSkillupList(
                        player->GetGUIDLow());
            }
            else if (itemTarget)
            {
    ...
    ExecuteLogEffectOpenLock(effIndex,
            gameObjTarget ? (Object*) gameObjTarget : (Object*) itemTarget);
}
...
void Spell::EffectSummonChangeItem(SpellEffIndex effIndex)
{
    ...
    if (m_CastItem->GetOwnerGUID() != player->GetGUID()) return;
    ...
    for (uint8 j = PERM_ENCHANTMENT_SLOT; j <= TEMP_ENCHANTMENT_SLOT; ++j)
        if (m_CastItem->GetEnchantmentId(EnchantmentSlot(j))) pNewItem->SetEnchantment(
                EnchantmentSlot(j),
                m_CastItem->GetEnchantmentId(EnchantmentSlot(j)),
                m_CastItem->GetEnchantmentDuration(EnchantmentSlot(j)),
                m_CastItem->GetEnchantmentCharges(EnchantmentSlot(j)));

    if (m_CastItem->GetUInt32Value(ITEM_FIELD_DURABILITY)
            < m_CastItem->GetUInt32Value(ITEM_FIELD_MAXDURABILITY))
    {
        double loosePercent = 1
                - m_CastItem->GetUInt32Value(ITEM_FIELD_DURABILITY)
                        / double(
    ...
    {
        ItemPosCountVec dest;
        uint8 msg = player->CanStoreItem(m_CastItem->GetBagSlot(),
                m_CastItem->GetSlot(), dest, pNewItem, true);
        if (msg == EQUIP_ERR_OK)
        {
            player->DestroyItem(m_CastItem->GetBagSlot(), m_CastItem->GetSlot(),
                    true);
    ...
    {
        ItemPosCountVec dest;
        uint8 msg = player->CanBankItem(m_CastItem->GetBagSlot(),
                m_CastItem->GetSlot(), dest, pNewItem, true);
        if (msg == EQUIP_ERR_OK)
        {
            player->DestroyItem(m_CastItem->GetBagSlot(), m_CastItem->GetSlot(),
    ...
    else if (player->IsEquipmentPos(pos))
    {
        ...
        uint8 msg = player->CanEquipItem(m_CastItem->GetSlot(), dest, pNewItem,
                true);

        if (msg == EQUIP_ERR_OK || msg == EQUIP_ERR_CANT_DO_RIGHT_NOW)
        {
            if (msg == EQUIP_ERR_CANT_DO_RIGHT_NOW) dest =
...
void Spell::EffectProficiency(SpellEffIndex /*effIndex*/)
{
            ...
            && !(p_target->GetWeaponProficiency() & subClassMask))
    {
        ...
        p_target->SendProficiency(ITEM_CLASS_WEAPON,
                p_target->GetWeaponProficiency());
    }
    if (m_spellInfo->EquippedItemClass == ITEM_CLASS_ARMOR
            && !(p_target->GetArmorProficiency() & subClassMask))
    {
        p_target->AddArmorProficiency(subClassMask);
        p_target->SendProficiency(ITEM_CLASS_ARMOR,
                p_target->GetArmorProficiency());
    }
}
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    switch (properties->Category)
    {
        default:
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_MINIPET:
                {
                    summon = m_caster->GetMap()->SummonCreature(entry, pos,
                            properties, duration, m_originalCaster);
                    if (!summon || !summon->HasUnitTypeMask(UNIT_MASK_MINION)) return;
                    ...
                    summon->SetUInt32Value(UNIT_NPC_FLAGS,
                            summon->GetCreatureInfo()->npcflag);
        ...
        case SUMMON_CATEGORY_PUPPET:
            summon = m_caster->GetMap()->SummonCreature(entry, pos, properties,
                    duration, m_originalCaster);
            break;
        case SUMMON_CATEGORY_VEHICLE:
        {
            ...
            summon = m_caster->GetMap()->SummonCreature(entry, pos, properties,
                    duration, m_caster);
            if (!summon || !summon->IsVehicle()) return;

            if (m_spellInfo->EffectBasePoints [effIndex])
            {
                SpellEntry const *spellProto = sSpellStore.LookupEntry(
                        SpellMgr::CalculateSpellEffectAmount(m_spellInfo,
                                effIndex));
                if (spellProto) m_caster->CastSpell(summon, spellProto, true);
            }
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex)
{
    ...
    {
        if (m_caster->GetTypeId() == TYPEID_PLAYER) EffectLearnPetSpell(
                effIndex);
    ...
    uint32 spellToLearn =
            (m_spellInfo->Id == 483 || m_spellInfo->Id == 55884) ?
                    damage : m_spellInfo->EffectTriggerSpell [effIndex];
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
    ...
    if (dispelMask & (1 << DISPEL_DISEASE) && unitTarget->HasAura(50536)) dispelMask &=
            ~(1 << DISPEL_DISEASE);
            ...
            ++itr)
    {
        ...
        if ((1 << aura->GetSpellProto()->Dispel) & dispelMask)
        {
            if (aura->GetSpellProto()->Dispel == DISPEL_MAGIC
                    || aura->GetSpellProto()->Dispel == DISPEL_POISON)
            {
                bool positive =
                        aurApp->IsPositive() ?
                ...
                if (positive == unitTarget->IsFriendlyTo(m_caster)) continue;
            }
            ...
            if (charges > 0) dispel_list.push_back(
                    std::make_pair(aura, charges));
        }
    }

    if (dispel_list.empty()) return;
    ...
    if (failCount) m_caster->SendMessageToSet(&dataFail, true);
    ...
    {
        int32 heal_amount = SpellMgr::CalculateSpellEffectAmount(m_spellInfo,
                1);
        m_caster->CastCustomSpell(m_caster, 19658, &heal_amount, NULL, NULL,
                true);
        // Glyph of Felhunter
        if (Unit* pOwner = m_caster->GetOwner()) if (pOwner->GetAura(56249)) pOwner->CastCustomSpell(
                pOwner, 19658, &heal_amount, NULL, NULL, true);
    }
}
...
void Spell::EffectDualWield(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget->GetTypeId() == TYPEID_UNIT) unitTarget->ToCreature()->UpdateDamagePhysical(
            OFF_ATTACK);
}
...
void Spell::EffectDistract(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || unitTarget->isInCombat()) return;

    // target must be OK to do this
    if (unitTarget->HasUnitState(
            UNIT_STAT_CONFUSED | UNIT_STAT_STUNNED | UNIT_STAT_FLEEING)) return;
    ...
    if (unitTarget->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        unitTarget->ToPlayer()->SetPosition(unitTarget->GetPositionX(),
...
void Spell::EffectPickPocket(SpellEffIndex /*effIndex*/)
{
    ...
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT
            || m_caster->IsFriendlyTo(unitTarget)) return;

    // victim have to be alive and humanoid or undead
    if (unitTarget->isAlive()
            && (unitTarget->GetCreatureTypeMask()
                    & CREATURE_TYPEMASK_HUMANOID_OR_UNDEAD) != 0) m_caster->ToPlayer()->SendLoot(
            unitTarget->GetGUID(), LOOT_PICKPOCKETING);
}
...
{
    if (!unitTarget || m_caster->GetTypeId() == TYPEID_PLAYER) return;
...
void Spell::EffectTeleUnitsFaceCaster(SpellEffIndex effIndex)
{
    ...
    if (unitTarget->isInFlight()) return;

    float dis = (float) m_caster->GetSpellRadiusForTarget(
            unitTarget,
            sSpellRadiusStore.LookupEntry(
                    m_spellInfo->EffectRadiusIndex [effIndex]));
    ...
    unitTarget->NearTeleportTo(fx, fy, fz, -m_caster->GetOrientation(),
            unitTarget == m_caster);
}
...
void Spell::EffectLearnSkill(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->SetSkill(skillid,
            SpellMgr::CalculateSpellEffectAmount(m_spellInfo, effIndex),
            skillval ? skillval : 1, damage * 75);
}
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_CastItem)
    {
        ...
        sLog->outDebug(
                LOG_FILTER_SPELLS_AURAS,
                "SpellEffect::AddHonor (spell_id %u) rewards %d honor points (item %u) for player: %u",
                m_spellInfo->Id, damage / 10, m_CastItem->GetEntry(),
                unitTarget->ToPlayer()->GetGUIDLow());
        return;
    }
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = Trinity::Honor::hk_honor_at_level(
                unitTarget->getLevel(), float(damage));
        unitTarget->ToPlayer()->RewardHonor(NULL, 1, honor_reward);
        sLog->outDebug(
                LOG_FILTER_SPELLS_AURAS,
                "SpellEffect::AddHonor (spell_id %u) rewards %u honor points (scale) to player: %u",
                m_spellInfo->Id, honor_reward,
                unitTarget->ToPlayer()->GetGUIDLow());
    }
    else
    {
        ...
        sLog->outDebug(
                LOG_FILTER_SPELLS_AURAS,
                "SpellEffect::AddHonor (spell_id %u) rewards %u honor points (non scale) for player: %u",
                m_spellInfo->Id, damage, unitTarget->ToPlayer()->GetGUIDLow());
    }
}
    ...
    else
    {
        ...
        if (!(m_CastItem
                && m_CastItem->GetProto()->Flags
                        & ITEM_PROTO_FLAG_TRIGGERED_CAST)) p_caster->UpdateCraftSkill(
                m_spellInfo->Id);

        uint32 enchant_id = m_spellInfo->EffectMiscValue [effIndex];
        if (!enchant_id) return;
    ...
    // support only enchantings with add socket in this slot
    {
        ...
        if (!add_socket)
        {
            sLog->outError(
                    ...
                    m_spellInfo->Id, SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC,
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN
            && m_spellInfo->SpellFamilyFlags [0] & 0x400000)
    {
        uint32 spell_id = 0;
        ...
        if (!spellInfo)
        {
            sLog->outError("Spell::EffectEnchantItemTmp: unknown spell id %i",
                    spell_id);
            return;
        }
    ...
    if (m_spellInfo->Id == 38615) duration = 1800; // 30 mins
    // other rogue family enchantments always 1 hour (some have spell damage=0, but some have wrong data in EffBasePoints)
    else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_ROGUE) duration = 3600; // 1 hour
    // shaman family enchantments
    else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN) duration =
            1800; // 30 mins
    // other cases with this SpellVisual already selected
    else if (m_spellInfo->SpellVisual [0] == 215) duration = 1800; // 30 mins
    // some fishing pole bonuses except Glow Worm which lasts full hour
    else if (m_spellInfo->SpellVisual [0] == 563 && m_spellInfo->Id != 64401) duration =
            600; // 10 mins
    // shaman rockbiter enchantments
    else if (m_spellInfo->SpellVisual [0] == 0) duration = 1800; // 30 mins
    else if (m_spellInfo->Id == 29702) duration = 300; // 5 mins
    else if (m_spellInfo->Id == 37360) duration = 300; // 5 mins
    // default case
    else duration = 3600; // 1 hour
    ...
    itemTarget->SetEnchantment(TEMP_ENCHANTMENT_SLOT, enchant_id,
            duration * 1000, 0);
...
void Spell::EffectTameCreature(SpellEffIndex /*effIndex*/)
{
    ...
    if (creatureTarget->isPet()) return;

    if (m_caster->getClass() != CLASS_HUNTER) return;
    ...
    uint8 level =
            (creatureTarget->getLevel() < (m_caster->getLevel() - 5)) ?
                    (m_caster->getLevel() - 5) : creatureTarget->getLevel();
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    if (m_originalCaster)
    {
        if (m_originalCaster->GetTypeId() == TYPEID_PLAYER) owner =
                (Player*) m_originalCaster;
        else if (m_originalCaster->ToCreature()->isTotem()) owner =
                m_originalCaster->GetCharmerOrOwnerPlayerOrPlayerItself();
    }

    uint32 petentry = m_spellInfo->EffectMiscValue [effIndex];

    PetSlot slot = (PetSlot) m_spellInfo->EffectBasePoints [effIndex];
    if (petentry) slot = PET_SLOT_UNK_SLOT;

    if (!owner)
    {
        SummonPropertiesEntry const *properties =
                sSummonPropertiesStore.LookupEntry(67);
        if (properties) SummonGuardian(effIndex, petentry, properties);
        return;
    }
    ...
    if (OldSummon)
    {
        if (petentry == 0 || OldSummon->GetEntry() == petentry)
        {
            ...
            if (owner->GetTypeId() == TYPEID_PLAYER
                    && OldSummon->isControlled()) owner->ToPlayer()->PetSpellInitialize();

            return;
        }

        if (owner->GetTypeId() == TYPEID_PLAYER) owner->ToPlayer()->RemovePet(
                OldSummon, PET_SLOT_ACTUAL_PET_SLOT, false);
        else return;
    }

    float x, y, z;
    owner->GetClosePoint(x, y, z, owner->GetObjectSize());
    Pet* pet = owner->SummonPet(petentry, x, y, z, owner->GetOrientation(),
            SUMMON_PET, 0, slot);
    if (!pet) return;

    if (m_caster->GetTypeId() == TYPEID_UNIT)
    {
        if (m_caster->ToCreature()->isTotem()) pet->SetReactState(
                REACT_AGGRESSIVE);
        else pet->SetReactState(REACT_DEFENSIVE);
    }
    ...
    if (!new_name.empty()) pet->SetName(new_name);
...
void Spell::EffectLearnPetSpell(SpellEffIndex effIndex)
{
    ...
    SpellEntry const *learn_spellproto = sSpellStore.LookupEntry(
            m_spellInfo->EffectTriggerSpell [effIndex]);
    if (!learn_spellproto) return;
...
void Spell::EffectTaunt(SpellEffIndex /*effIndex*/)
{
    ...
    {
        int32 damageDone = int32(
                1 + m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.5f);
        bool is_crit = m_caster->isSpellCrit(unitTarget, m_spellInfo,
                m_spellSchoolMask, m_attackType);
        if (is_crit) damageDone *= 2;
        m_caster->DealDamage(unitTarget, damageDone, NULL, SPELL_DIRECT_DAMAGE,
                SPELL_SCHOOL_MASK_HOLY, m_spellInfo, false);
        m_caster->SendSpellNonMeleeDamageLog(unitTarget, m_spellInfo->Id,
                damageDone, SPELL_SCHOOL_MASK_HOLY, 0, 0, false, false,
                is_crit);
    }
    ...
    if (unitTarget->getThreatManager().getCurrentVictim())
    {
        ...
        if (itsThreat > myThreat) unitTarget->getThreatManager().addThreat(
                m_caster, itsThreat - myThreat);
    }

    //Set aggro victim to caster
    if (!unitTarget->getThreatManager().getOnlineContainer().empty()) if (HostileReference* forcedVictim = unitTarget->getThreatManager().getOnlineContainer().getReferenceByTarget(m_caster)) unitTarget->getThreatManager().setCurrentVictim(
            forcedVictim);

    if (unitTarget->ToCreature()->IsAIEnabled
            && !unitTarget->ToCreature()->HasReactState(REACT_PASSIVE)) unitTarget->ToCreature()->AI()->AttackStart(
            m_caster);
}
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 8676:
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER) if (Item* item = m_caster->ToPlayer()->GetWeaponForAttack(m_attackType, true)) if (item->GetProto()->SubClass
                    == ITEM_SUBCLASS_WEAPON_DAGGER) damage +=
                    m_spellInfo->EffectBasePoints [1];
        }
        case 20243:
        {
...
void Spell::SpellDamageWeaponDmg(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags [1] & 0x40)
            {
                ...
                if (needCast) m_caster->CastSpell(unitTarget, 58567, true);

                if (Aura * aur = unitTarget->GetAura(58567))
                {
                    // 58388 - Glyph of Devastate dummy aura.
                    if (int32 num = (needCast ? 0 : 1) + (m_caster->HasAura(58388) ? 1 : 0)) aur->ModStackAmount(
                            num);
                    fixed_bonus += (aur->GetStackAmount() - 1)
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            else if (m_spellInfo->SpellFamilyFlags [1] & 0x6)
            {
                ...
                if (unitTarget->HasAuraState(AURA_STATE_DEADLY_POISON,
                        m_spellInfo, m_caster)) found = true;
                // full aura scan
                else
                {
                    Unit::AuraApplicationMap const& auras =
                            unitTarget->GetAppliedAuras();
                    for (Unit::AuraApplicationMap::const_iterator itr =
                            auras.begin(); itr != auras.end(); ++itr)
                    {
                        if (itr->second->GetBase()->GetSpellProto()->Dispel
                                == DISPEL_POISON)
                        {
                            found = true;
                            break;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            // Stormstrike
            if (AuraEffect * aurEff = m_caster->IsScriptOverriden(m_spellInfo, 5634)) m_caster->CastSpell(
                    m_caster, 38430, true, NULL, aurEff);
            break;
        }
        case SPELLFAMILY_DRUID:
        {
            ...
            else if (m_spellInfo->SpellFamilyFlags [0] & 0x00008800
                    && unitTarget->HasAuraState(AURA_STATE_BLEEDING))
            {
                ...
                {
                    totalDamagePercentMod *= float(
                            (rendAndTear->GetAmount() + 100.0f) / 100.0f);
                }
            }
            break;
        }
        case SPELLFAMILY_HUNTER:
        {
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 53209: // Chimera Shot
                {
                    ...
                    if (m_spellInfo->SpellFamilyFlags [2] & 0x1) shotMod =
                            0.732f;

                    // Marked for Death 1, 2
                    if (m_caster->HasAuraEffect(53241, 0, 0)) if (roll_chance_i(
                            sSpellStore.LookupEntry(53241)->EffectBasePoints [0]))
                    {
                        m_caster->CastSpell(
                                m_caster->ToPlayer()->GetSelectedUnit(), 88691,
                                true);
                        break;
                    }
                    if (m_caster->HasAuraEffect(53243, 0, 0)) if (roll_chance_i(
                            sSpellStore.LookupEntry(53243)->EffectBasePoints [0]))
                    {
                        m_caster->CastSpell(
                                m_caster->ToPlayer()->GetSelectedUnit(), 88691,
                                true);
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags [EFFECT_0] & 0x1)
            {
                ...
                if (AuraEffect const * aurEff = m_caster->GetAuraEffect(58657, EFFECT_0)) totalDamagePercentMod *=
                        (aurEff->GetAmount() + 100.0f) / 100.0f;
                break;
            }
            // Blood Strike
            if (m_spellInfo->SpellFamilyFlags [EFFECT_0] & 0x400000)
            {
                ...
                if (m_caster->GetAuraEffect(59332, EFFECT_0)) if (unitTarget->HasAuraType(
                        SPELL_AURA_MOD_DECREASE_SPEED)) totalDamagePercentMod *=
                        (20 + 100.0f) / 100.0f;
                break;
            }
            // Death Strike
            if (m_spellInfo->SpellFamilyFlags [EFFECT_0] & 0x10)
            {
                ...
                if (AuraEffect const * aurEff = m_caster->GetAuraEffect(59336, EFFECT_0)) if (uint32 runic = std::min<uint32>(m_caster->GetPower(POWER_RUNIC_POWER), SpellMgr::CalculateSpellEffectAmount(aurEff->GetSpellProto(), EFFECT_1))) totalDamagePercentMod *=
                        ((runic * 0.4f) + 100.0f) / 100.0f;
                break;
            }
            ...
            {
                totalDamagePercentMod *= ((unitTarget->GetDiseasesByCaster(
                        m_caster->GetGUID()) * 12.5f) + 100.0f) / 100.0f;
                break;
            }
            // Heart Strike
    ...
    if (fixed_bonus || spell_bonus)
    {
        ...
        if (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_NORMAL) weapon_total_pct =
                m_caster->GetModifierValue(unitMod, TOTAL_PCT);
        ...
        if (fixed_bonus)
        {
            ...
            if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT
                    && m_caster->ToPlayer())
            {
                if (m_spellInfo->SpellFamilyFlags [0] & 0x11
                        || m_spellInfo->SpellFamilyFlags [1] & 0x8000000)
                {
                    float coeff = 1.0f;
                    m_caster->ToPlayer()->ApplySpellMod(m_spellInfo->Id,
                            SPELLMOD_DAMAGE, coeff);
                    fixed_bonus *= coeff;
                }
            }
        }
...
{
    if (!unitTarget || !unitTarget->isAlive() || !m_caster->isAlive()) return;

    if (!unitTarget->CanHaveThreatList()) return;
...
void Spell::EffectHealMaxHealth(SpellEffIndex effIndex)
{
    ...
    if (damage == 0) addhealth = m_caster->GetMaxHealth();
    else addhealth = unitTarget->GetMaxHealth() - unitTarget->GetHealth();

    if (m_originalCaster) m_healing += m_originalCaster->SpellHealingBonus(
            unitTarget, m_spellInfo, effIndex, addhealth, HEAL);
}
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    float x, y, z;
    if (m_targets.HasDst()) m_targets.m_dstPos.GetPosition(x, y, z);
    else m_caster->GetClosePoint(x, y, z, DEFAULT_WORLD_OBJECT_SIZE);
        ...
        switch (pGameObj->GetMapId())
        {
            case 489: //WS
            {
                        ...
                        && bg->GetStatus() == STATUS_IN_PROGRESS)
                {
                    ...
                    if (pl->GetTeam() == team) team = HORDE;

                    ((BattlegroundWS*) bg)->SetDroppedFlagGUID(
                            pGameObj->GetGUID(), team);
                }
                break;
            }
            case 566: //EY
            {
                if (bg && bg->GetTypeID(true) == BATTLEGROUND_EY
                        && bg->GetStatus() == STATUS_IN_PROGRESS)
                {
                    ((BattlegroundEY*) bg)->SetDroppedFlagGUID(
                            pGameObj->GetGUID());
                }
                break;
            }
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 58622:
                {
                    if (OutdoorPvPWG * pvpWG =
                            (OutdoorPvPWG*) sOutdoorPvPMgr->GetOutdoorPvPToZoneId(
                                    4197)) if (pvpWG->isWarTime()
                            || pvpWG->m_timer < 300000)
                    {
                        if ((pvpWG->getDefenderTeam() == TEAM_ALLIANCE)
                                && (unitTarget->ToPlayer()->GetTeam()
                                        == ALLIANCE)) unitTarget->CastSpell(
                                unitTarget, SPELL_TELEPORT_FORTRESS, true);
                        else if ((pvpWG->getDefenderTeam() == TEAM_ALLIANCE)
                                && (unitTarget->ToPlayer()->GetTeam() == HORDE)) unitTarget->CastSpell(
                                unitTarget, SPELL_TELEPORT_HORDE_CAMP, true);
                        ...
                        if ((pvpWG->getDefenderTeam() != TEAM_ALLIANCE)
                                ...
                                && (unitTarget->ToPlayer()->GetTeam()
                                        == ALLIANCE)) unitTarget->CastSpell(
                                unitTarget, SPELL_TELEPORT_ALLIENCE_CAMP, true);
                    }
                    return;
                }
                    // Glyph of Backstab
                ...
                case 35376:
                    if (unitTarget) unitTarget->ToPlayer()->TeleportTo(0,
                            1805.99f, 341.32f, 70.66f, 1.6f);
                    break;
                case 35727:
                    if (unitTarget) unitTarget->ToPlayer()->TeleportTo(530,
                            10038.7f, -7000.9f, 61.86f, 3.05f);
                    break;
                case 45204: // Clone Me!
                ...
                case 55693: // Remove Collapsing Cave Aura
                    if (!unitTarget) return;
                    unitTarget->RemoveAurasDueToSpell(
                            SpellMgr::CalculateSpellEffectAmount(m_spellInfo,
                                    effIndex));
                    break;
                ...
                {
                    uint32 spells [4] =
                    { 26272, 26157, 26273, 26274 };

                    // check presence
                    for (uint8 j = 0; j < 4; ++j)
                        if (unitTarget->HasAuraEffect(spells [j], 0)) return;
                ...
                case 8856:
                {
                    if (!itemTarget && m_caster->GetTypeId() != TYPEID_PLAYER) return;
                ...
                case 22985:
                {
                    ...
                    // Onyxia Scale Cloak
                    if (unitTarget->HasAura(22683)) return;
                ...
                case 54426:
                    if (unitTarget)
                    {
                        ...
                        if (damage > 0) m_caster->CastCustomSpell(28375,
                                SPELLVALUE_BASE_POINT0, damage, unitTarget);
                    }
                    return;
                ...
                case 29830:
                {
                    ...
                    if (item) DoCreateItem(effIndex, item);
                    break;
                }
                    // Improved Sprint
                case 30918:
                ...
                case 48025: // Headless Horseman's Mount
                {
                    ...
                    bool canFly = true;
                    uint32 v_map = GetVirtualMapForMapAndZone(
                            unitTarget->GetMapId(), unitTarget->GetZoneId());
                    if (v_map != 530 && v_map != 571 && v_map != 0) canFly =
                            false;

                    if (canFly && v_map == 571
                            && !unitTarget->ToPlayer()->HasSpell(54197)) canFly =
                            false;

                    if (canFly && v_map == 0
                            && !unitTarget->ToPlayer()->HasSpell(90267)) canFly =
                            false;

                    float x, y, z;
                    unitTarget->GetPosition(x, y, z);
                    uint32 areaFlag = unitTarget->GetBaseMap()->GetAreaFlag(x,
                            y, z);
                    AreaTableEntry const *pArea = sAreaStore.LookupEntry(
                            areaFlag);
                    if (canFly
                            && (pArea && pArea->flags & AREA_FLAG_NO_FLY_ZONE)
                            || !pArea) canFly = false;

                    switch (unitTarget->ToPlayer()->GetBaseSkillValue(
                            SKILL_RIDING))
                    {
                        case 75:
                        ...
                        case 225:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 51617,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 48024, true);
                        }
                            break;
                        case 300:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 48023,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 48024, true);
                        }
                            break;
                ...
                case 47977: // Magic Broom
                {
                            ...
                            SKILL_RIDING))
                    {
                        ...
                        case 225:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 42667,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 42683, true);
                        }
                            break;
                        case 300:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 42668,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 42683, true);
                        }
                            break;
                ...
                case 41931:
                {
                    ...
                    if (bag)
                    {
                        if (m_caster->ToPlayer()->GetItemByPos(bag, slot)->GetCount()
                                == 1) m_caster->ToPlayer()->RemoveItem(bag,
                                slot, true);
                        else m_caster->ToPlayer()->GetItemByPos(bag, slot)->SetCount(
                                m_caster->ToPlayer()->GetItemByPos(bag, slot)->GetCount()
                                        - 1);
                ...
                case 45151:
                {
                    ...
                    if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER
                            && unitTarget->GetDistance(m_caster) >= radius
                            && !unitTarget->HasAura(46394)
                            && unitTarget != m_caster) unitTarget->CastSpell(
                            unitTarget, 46394, true);
                ...
                case 47770:
                {
                    ...
                    if (m_caster->getGender() > 0) gender = "her";
                    sprintf(
                            buf,
                            "%s rubs %s [Decahedral Dwarven Dice] between %s hands and rolls. One %u and one %u.",
                            m_caster->GetName(), gender, gender, urand(1, 10),
                            urand(1, 10));
                ...
                case 47776:
                {
                    ...
                    sprintf(
                            buf,
                            "%s causually tosses %s [Worn Troll Dice]. One %u and one %u.",
                            m_caster->GetName(), gender, urand(1, 6),
                            urand(1, 6));
                    m_caster->MonsterTextEmote(buf, 0);
                    break;
                }
                ...
                case 51770:
                {
                    ...
                    m_originalCaster->CastSpell(m_originalCaster, damage,
                            false);
                    break;
                }
                ...
                {
                    if (!unitTarget) return;
                    if (unitTarget->HasAura(51845)) unitTarget->CastSpell(
                            m_caster, 51856, true);
                    else m_caster->CastSpell(unitTarget, 51855, true);
                    break;
                }
                    // Summon Ghouls On Scarlet Crusade
                case 51904:
                {
                    if (!m_targets.HasDst()) return;

                    float x, y, z;
                    float radius = GetSpellRadius(m_spellInfo, effIndex, true);
                    for (uint8 i = 0; i < 15; ++i)
                    {
                        m_caster->GetRandomPoint(m_targets.m_dstPos, radius, x,
                                y, z);
                ...
                case 60243: // Blood Parrot Despawn
                    if (unitTarget->GetTypeId() == TYPEID_UNIT
                            && unitTarget->ToCreature()->isSummon()) unitTarget->ToTempSummon()->UnSummon();
                    return;
                case 52479: // Gift of the Harvester
                    if (unitTarget && m_originalCaster) m_originalCaster->CastSpell(
                            unitTarget, urand(0, 1) ? damage : 52505, true);
                    return;
                    // Death Gate
                case 52751:
                {
                    if (!unitTarget
                            || unitTarget->getClass() != CLASS_DEATH_KNIGHT) return;
                    // triggered spell is stored in m_spellInfo->EffectBasePoints[0]
                    unitTarget->CastSpell(unitTarget, damage, false);
                    break;
                }
                case 53110: // Devour Humanoid
                    if (unitTarget) unitTarget->CastSpell(m_caster, damage,
                            true);
                    return;
                ...
                case 54729:
                {
                    ...
                    {
                        if (skillval >= 300) unitTarget->CastSpell(unitTarget,
                                54727, true);
                        else unitTarget->CastSpell(unitTarget, 54726, true);
                    }
                    return;
                }
                case 58418: // Portal to Orgrimmar
                case 58420: // Portal to Stormwind
                {
                    ...
                    uint32 spellID = SpellMgr::CalculateSpellEffectAmount(
                            m_spellInfo, 0);
                    uint32 questID = SpellMgr::CalculateSpellEffectAmount(
                            m_spellInfo, 1);

                    if (unitTarget->ToPlayer()->GetQuestStatus(questID)
                            == QUEST_STATUS_COMPLETE) unitTarget->CastSpell(
                            unitTarget, spellID, true);
                ...
                case 58941: // Rock Shards
                    if (unitTarget && m_originalCaster)
                    {
                        ...
                        {
                            m_originalCaster->CastSpell(unitTarget, 58689,
                                    true);
                            m_originalCaster->CastSpell(unitTarget, 58692,
                                    true);
                        }
                        if (((InstanceMap*) m_originalCaster->GetMap())->GetDifficulty()
                                == REGULAR_DIFFICULTY)
                        {
                            m_originalCaster->CastSpell(unitTarget, 58695,
                                    true);
                            m_originalCaster->CastSpell(unitTarget, 58696,
                                    true);
                        }
                        else
                        {
                            m_originalCaster->CastSpell(unitTarget, 60883,
                                    true);
                            m_originalCaster->CastSpell(unitTarget, 60884,
                                    true);
                        }
                    }
                    return;
                case 58983: // Big Blizzard Bear
                {
                    ...
                    if (uint16 skillval = unitTarget->ToPlayer()->GetSkillValue(SKILL_RIDING))
                    {
                        if (skillval >= 150) unitTarget->CastSpell(unitTarget,
                                58999, true);
                        else unitTarget->CastSpell(unitTarget, 58997, true);
                    }
                    return;
                }
                case 63845: // Create Lance
                {
                    ...
                    if (m_caster->ToPlayer()->GetTeam() == ALLIANCE) m_caster->CastSpell(
                            m_caster, 63914, true);
                    else m_caster->CastSpell(m_caster, 63919, true);
                    return;
                }
                case 71342: // Big Love Rocket
                {
                    ...
                    AreaTableEntry const *pArea = sAreaStore.LookupEntry(
                            areaFlag);
                    if (!pArea
                            || (canFly && (pArea->flags & AREA_FLAG_NO_FLY_ZONE))) canFly =
                            false;
                    ...
                    {
                        case 0:
                            unitTarget->CastSpell(unitTarget, 71343, true);
                            break;
                        case 75:
                        ...
                        case 225:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 71346,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 71345, true);
                        }
                            break;
                        case 300:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 71347,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 71345, true);
                        }
                            break;
                ...
                case 72286: // Invincible
                {
                            ...
                            SKILL_RIDING))
                    {
                        ...
                        case 225:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 72283,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 72282, true);
                        }
                            break;
                        case 300:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 72284,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 72282, true);
                        }
                            break;
                ...
                case 74856: // Blazing Hippogryph
                {
                    ...
                    if (uint16 skillval = unitTarget->ToPlayer()->GetSkillValue(SKILL_RIDING))
                    {
                        if (skillval >= 300) unitTarget->CastSpell(unitTarget,
                                74855, true);
                        else unitTarget->CastSpell(unitTarget, 74854, true);
                    }
                    return;
                }
                case 75614: // Celestial Steed
                {
                            ...
                            SKILL_RIDING))
                    {
                        ...
                        case 225:
                        {
                            if (canFly) unitTarget->CastSpell(unitTarget, 75617,
                                    true);
                            else unitTarget->CastSpell(unitTarget, 75620, true);
                        }
                            break;
                        case 300:
                        {
                            if (canFly)
                            {
                                if (unitTarget->ToPlayer()->Has310Flyer(false)) unitTarget->CastSpell(
                                        unitTarget, 76153, true);
                                else unitTarget->CastSpell(unitTarget, 75618,
                                        true);
                            }
                            else unitTarget->CastSpell(unitTarget, 75620, true);
                        }
                ...
                case 75973: // X-53 Touring Rocket
                {
                    ...
                    if (uint16 skillval = unitTarget->ToPlayer()->GetSkillValue(SKILL_RIDING))
                    {
                        ...
                        {
                            if (unitTarget->ToPlayer()->Has310Flyer(false)) unitTarget->CastSpell(
                                    unitTarget, 76154, true);
                            else unitTarget->CastSpell(unitTarget, 75972, true);
                        }
                        else unitTarget->CastSpell(unitTarget, 75957, true);
                    }
                    return;
                }
                case 59317: // Teleporting
                    ...
                    // return from top
                    if (unitTarget->ToPlayer()->GetAreaId() == 4637) unitTarget->CastSpell(
                            unitTarget, 59316, true);
                    // teleport atop
                    else unitTarget->CastSpell(unitTarget, 59314, true);
                ...
                case 64323: // Book of Glyph Mastery
                {
                    ...
                    if (uint32 discoveredSpell = GetExplicitDiscoverySpell(m_spellInfo->Id, m_caster->ToPlayer())) m_caster->ToPlayer()->learnSpell(
                            discoveredSpell, false);
                    return;
                }
                case 62428: // Load into Catapult
                {
                    if (Vehicle *seat = m_caster->GetVehicleKit()) if (Unit *passenger = seat->GetPassenger(0)) if (Unit *demolisher = m_caster->GetVehicleBase()) passenger->CastSpell(
                            demolisher, damage, true);
                    return;
                }
                case 62482: // Grab Crate
                {
                    if (unitTarget)
                    {
                        ...
                        {
                            if (Creature *oldContainer = dynamic_cast<Creature*>(seat->GetPassenger(1))) oldContainer->DisappearAndDie();
                ...
                {
                    if (m_caster->GetTypeId() != TYPEID_UNIT
                            || !m_caster->ToCreature()->isSummon()) return;
                    ...
                    switch (m_caster->GetEntry())
                    {
                        ...
                        case 31883:
                            ...
                            break;
                        default:
                            sLog->outError("Unknown Lightwell spell caster %u",
                                    m_caster->GetEntry());
                            return;
                    }
                    ...
                    if (chargesaura && chargesaura->GetCharges() > 1)
                    {
                        ...
                        unitTarget->CastSpell(unitTarget, spell_heal, true,
                                NULL, NULL,
                                m_caster->ToTempSummon()->GetSummonerGUID());
                    }
                    else m_caster->ToTempSummon()->UnSummon();
                    return;
                }
                    // Stoneclaw Totem
                case 55328: // Rank 1
                ...
                case 58591: // Rank 10
                {
                    ...
                    for (uint8 slot = SUMMON_SLOT_TOTEM; slot < MAX_TOTEM_SLOT;
                            ++slot)
                    {
                        if (!unitTarget->m_SummonSlot [slot]) continue;

                        Creature* totem = unitTarget->GetMap()->GetCreature(
                                unitTarget->m_SummonSlot [slot]);
                        if (totem && totem->isTotem())
                        {
                            m_caster->CastCustomSpell(totem, 55277,
                    ...
                    if (AuraEffect *aur=unitTarget->GetAuraEffect(63298, 0))
                    {
                        ...
                        m_caster->CastCustomSpell(unitTarget, 55277,
                                &basepoints0, NULL, NULL, true);
                    }
                    break;
                }
                case 66545: //Summon Memory
                {
                    ...
                    uint32 uiSpells [26] =
                    { 66704, 66705, 66706, 66707, 66709, 66710, 66711, 66712,
                            66713, 66714, 66715, 66708, 66708, 66691, 66692,
                            66694, 66695, 66696, 66697, 66698, 66699, 66700,
                            66701, 66702, 66703, 66543 };
                ...
                case 45668: // Ultra-Advanced Proto-Typical Shortening Blaster
                {
                    ...
                    m_caster->CastSpell(m_caster, spellPlayer [urand(0, 4)],
                            true);
                    unitTarget->CastSpell(unitTarget, spellTarget [urand(0, 4)],
                            true);
                    break;
                }
                case 64142: // Upper Deck - Create Foam Sword
                    ...
                    static uint32 const itemId [] =
                    { 45061, 45176, 45177, 45178, 45179, 0 };
                    // player can only have one of these items
                    for (uint32 const *itr = &itemId [0]; *itr; ++itr)
                        if (plr->HasItemCount(*itr, 1, true)) return;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (m_spellInfo->Category == SPELLCATEGORY_JUDGEMENT)
            {
                ...
                Unit::AuraApplicationMap & sealAuras =
                        m_caster->GetAppliedAuras();
                for (Unit::AuraApplicationMap::iterator iter =
                        sealAuras.begin(); iter != sealAuras.end();)
                {
                    Aura * aura = iter->second->GetBase();
                    if (IsSealSpell(aura->GetSpellProto()))
                    {
                        if (AuraEffect * aureff = aura->GetEffect(2)) if (aureff->GetAuraType()
                                == SPELL_AURA_DUMMY)
                        {
                            if (sSpellStore.LookupEntry(aureff->GetAmount())) spellId =
                                    aureff->GetAmount();
                            break;
                        }
                        if (!spellId)
                ...
                // Cast Judgement
                if (spellId) m_caster->CastSpell(unitTarget, spellId, true);

                // Check for Judgement dependent auras
                Unit::AuraApplicationMap & talentauras =
                        m_caster->GetAppliedAuras();
                for (Unit::AuraApplicationMap::iterator iter =
                        talentauras.begin(); iter != talentauras.end();)
                {
                    Aura * aura = iter->second->GetBase();
                    switch (aura->GetId())
                    {
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags [2] & 0x1)
            {
                ...
                if (unitTarget->GetAura(1978)) unitTarget->GetAura(1978)->RefreshDuration();
            }
            return;
        }
        case SPELLFAMILY_POTION:
        {
            switch (m_spellInfo->Id)
            {
                ...
                case 28702:
                {
                    ...
                    for (uint8 i = 0; i < 5; ++i)
                        if (unitTarget->HasAura(spellid + i)) return;
                    unitTarget->CastSpell(unitTarget, spellid + urand(0, 4),
                            true);
                    break;
                }
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags [1] & 0x10000)
            {
                ...
                if (m_targets.getUnitTarget() && // Glyph of Disease - cast on unit target too to refresh aura
                        (m_targets.getUnitTarget() != unitTarget
                                || m_caster->GetAura(63334)))
                {
                    // And spread them on target
                    // Blood Plague
                    if (m_targets.getUnitTarget()->GetAura(55078)) m_caster->CastSpell(
                            unitTarget, 55078, true);
                    // Frost Fever
                    if (m_targets.getUnitTarget()->GetAura(55095)) m_caster->CastSpell(
                            unitTarget, 55095, true);
                }
            }
            ...
            if (m_spellInfo->Id == 85948)
            {
                ...
                if (unitTarget->HasAura(45524)) // Chains of Ice
                {
                    ...
                    unitTarget->GetAura(45524)->SetDuration(
                            (unitTarget->GetAura(45524)->GetDuration()
                                    + r * 1000), true);
                }
                if (unitTarget->HasAura(55095)) // Frost Fever
                {
                    unitTarget->GetAura(55095)->RefreshDuration();
                    unitTarget->GetAura(55095)->SetDuration(
                            (unitTarget->GetAura(55095)->GetDuration()
                                    + r * 1000), true);
                }
                if (unitTarget->HasAura(55078)) // Blood Plague
                {
                    unitTarget->GetAura(55078)->RefreshDuration();
                    unitTarget->GetAura(55078)->SetDuration(
                            (unitTarget->GetAura(55078)->GetDuration()
                                    + r * 1000), true);
                }
            }
            break;
    ...
    // normal DB scripted effect
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS,
            "Spell ScriptStart spellid %u in EffectScriptEffect(%u)",
            m_spellInfo->Id, effIndex);
    m_caster->GetMap()->ScriptsStart(sSpellScripts,
            uint32(m_spellInfo->Id | (effIndex << 24)), m_caster, unitTarget);
}
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/)
{
    ...
    for (Unit::AttackerSet::const_iterator itr = attackers.begin();
            itr != attackers.end();)
    {
        if (!(*itr)->canSeeOrDetect(unitTarget)) (*(itr++))->AttackStop();
        else ++itr;
    }
            ...
            && (m_spellInfo->SpellFamilyFlags [0] & SPELLFAMILYFLAG_ROGUE_VANISH))
    {
        ...
        // Overkill
        if (m_caster->ToPlayer()->HasSpell(58426)) m_caster->CastSpell(m_caster,
                58427, true);
    }
}
...
void Spell::EffectAddComboPoints(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_caster->m_movedPlayer) return;

    Player* plr = m_caster->m_movedPlayer;

    if (damage > 0) plr->AddComboPoints(unitTarget, damage, this);
    else
    {
        // Rogue: Redirect
        if (GetSpellInfo()->Id == 73981 && plr->GetComboPoints() > 0
                && plr->GetComboTarget()) plr->AddComboPoints(unitTarget,
                plr->GetComboPoints(), this);

        // Rogue: Sinister Strike Enabler
        if (plr->getLevel() >= 3) plr->learnSpell(79327, false);
    }
}
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if (caster->duel || target->duel || !target->GetSocial()
            || target->GetSocial()->HasIgnore(caster->GetGUIDLow())) return;
    ...
    AreaTableEntry const* casterAreaEntry = GetAreaEntryByAreaID(
            caster->GetZoneId());
    ...
    AreaTableEntry const* targetAreaEntry = GetAreaEntryByAreaID(
            target->GetZoneId());
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_CAST_UNSTUCK)) return;
    ...
    sLog->outDetail(
            ...
            m_caster->GetPositionX(), pTarget->GetPositionY(),
            pTarget->GetPositionZ());

    if (pTarget->isInFlight()) return;

    pTarget->TeleportTo(pTarget->GetStartPosition(),
            unitTarget == m_caster ? TELE_TO_SPELL : 0);
...
void Spell::EffectSummonPlayer(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget->HasAura(23445)) return;
...
void Spell::EffectActivateObject(SpellEffIndex effIndex)
{
    ...
    gameObjTarget->GetMap()->ScriptCommandStart(activateCommand, delay_secs,
            m_caster, gameObjTarget);
}

void Spell::EffectApplyGlyph(SpellEffIndex effIndex)
{
    if (m_caster->GetTypeId()
            != TYPEID_PLAYER || m_glyphIndex >= MAX_GLYPH_SLOT_INDEX) return;
...
void Spell::EffectEnchantHeldItem(SpellEffIndex effIndex)
{
    ...
    Item* item = item_owner->GetItemByPos(INVENTORY_SLOT_BAG_0,
            EQUIPMENT_SLOT_MAINHAND);
    ...
    if (m_spellInfo->EffectMiscValue [effIndex])
    {
        ...
        if (!duration) duration = damage; //+1;            //Base points after ..
        if (!duration) duration = 10; //10 seconds for enchants which don't have listed duration
        ...
        if (item->GetEnchantmentId(slot)
                && item->GetEnchantmentId(slot) != enchant_id) return;
...
void Spell::EffectDisEnchant(SpellEffIndex /*effIndex*/)
{
    ...
    Player* p_caster = (Player*) m_caster;
    if (!itemTarget || !itemTarget->GetProto()->DisenchantID) return;
...
void Spell::EffectInebriate(SpellEffIndex /*effIndex*/)
{
    ...
    if (currentDrunk + drunkMod > 0xFFFF) currentDrunk = 0xFFFF;
    else currentDrunk += drunkMod;
...
void Spell::EffectFeedPet(SpellEffIndex effIndex)
{
    ...
    int32 benefit = pet->GetCurrentFoodBenefitLevel(
    ...
    m_caster->CastCustomSpell(pet, m_spellInfo->EffectTriggerSpell [effIndex],
            &benefit, NULL, NULL, true);
}
...
void Spell::EffectSummonObject(SpellEffIndex effIndex)
{
    ...
    {
        GameObject* obj = NULL;
        if (m_caster) obj = m_caster->GetMap()->GetGameObject(guid);

        if (obj)
        {
            // Recast case - null spell id to make auras not be removed on object remove from world
            if (m_spellInfo->Id == obj->GetSpellId()) obj->SetSpellId(0);
...
{
    if (!unitTarget || !unitTarget->isAlive() || !unitTarget->getVictim()) return;

    if (unitTarget->m_extraAttacks) return;
...
{
    if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER) unitTarget->ToPlayer()->SetCanParry(
            true);
}

void Spell::EffectBlock(SpellEffIndex /*effIndex*/)
{
    if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER) unitTarget->ToPlayer()->SetCanBlock(
            true);
}
...
void Spell::EffectLeap(SpellEffIndex /*effIndex*/)
{
    ...
    unitTarget->NearTeleportTo(m_targets.m_dstPos.GetPositionX(),
            m_targets.m_dstPos.GetPositionY(),
            m_targets.m_dstPos.GetPositionZ(),
...
void Spell::EffectQuestComplete(SpellEffIndex effIndex)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER) pPlayer = (Player*) m_caster;
    else if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER) pPlayer =
            (Player*) unitTarget;
    else return;
    ...
    if (quest_id)
    {
        ...
        if (log_slot < MAX_QUEST_LOG_SIZE) pPlayer->AreaExploredOrEventHappens(
                quest_id);
...
void Spell::EffectSelfResurrect(SpellEffIndex effIndex)
{
    ...
    else
    {
        ...
        if (unitTarget->GetMaxPower(POWER_MANA) > 0) mana = uint32(
                damage / 100.0f * unitTarget->GetMaxPower(POWER_MANA));
    }
...
void Spell::EffectSkinning(SpellEffIndex /*effIndex*/)
{
    ...
    int32 reqValue =
            targetLevel < 10 ? 0 :
            targetLevel < 20 ? (targetLevel - 10) * 10 : targetLevel * 5;

    int32 skillValue = m_caster->ToPlayer()->GetPureSkillValue(skill);

    // Double chances for elites
    m_caster->ToPlayer()->UpdateGatherSkill(skill, skillValue, reqValue,
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
    ...
    target->GetContactPoint(m_caster, pos.m_positionX, pos.m_positionY,
            pos.m_positionZ);
    target->GetFirstCollisionPosition(pos, target->GetObjectSize(), angle);

    m_caster->GetMotionMaster()->MoveCharge(pos.m_positionX, pos.m_positionY,
            pos.m_positionZ + target->GetObjectSize());
...
void Spell::EffectKnockBack(SpellEffIndex effIndex)
{
    ...
    if (unitTarget->IsNonMeleeSpellCasted(true)) unitTarget->InterruptNonMeleeSpells(
            true);

    // Typhoon
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_DRUID
            && m_spellInfo->SpellFamilyFlags [1] & 0x01000000)
    {
        // Glyph of Typhoon
        if (m_caster->HasAura(62135)) return;
    }

    // Thunderstorm
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN
            && m_spellInfo->SpellFamilyFlags [1] & 0x00002000)
    {
        // Glyph of Thunderstorm
        if (m_caster->HasAura(62132)) return;
    }

    float ratio = m_caster->GetCombatReach()
            / std::max(unitTarget->GetCombatReach(), 1.0f);
    if (ratio < 1.0f) ratio = ratio * ratio * ratio * 0.1f; // volume = length^3
    else ratio = 0.1f; // dbc value ratio
    float speedxy = float(m_spellInfo->EffectMiscValue [effIndex]) * ratio;
    float speedz = float(damage) * ratio;
    if (speedxy < 0.1f && speedz < 0.1f) return;

    float x, y;
    if (m_spellInfo->Effect [effIndex] == SPELL_EFFECT_KNOCK_BACK_DEST)
    {
        if (m_targets.HasDst()) m_targets.m_dstPos.GetPosition(x, y);
        else return;
    }
...
void Spell::EffectLeapBack(SpellEffIndex effIndex)
{
    ...
    if (!speedxy)
    {
        if (m_targets.getUnitTarget()) m_caster->JumpTo(
                m_targets.getUnitTarget(), speedz);
    }
    else
    {
        //1891: Disengage
...
{
    Player *pPlayer = NULL;
    if (m_caster->GetTypeId() == TYPEID_PLAYER) pPlayer = m_caster->ToPlayer();
    else if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER) pPlayer =
            unitTarget->ToPlayer();
    ...
    if (pPlayer->GetQuestStatus(quest_id) == QUEST_STATUS_NONE) return;
...
void Spell::EffectSendTaxi(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->ActivateTaxiPathTo(
            m_spellInfo->EffectMiscValue [effIndex], m_spellInfo->Id);
}
...
void Spell::EffectPullTowards(SpellEffIndex effIndex)
{
    ...
    float speedZ = (float) (SpellMgr::CalculateSpellEffectAmount(m_spellInfo,
            effIndex) / 10);
    float speedXY = (float) (m_spellInfo->EffectMiscValue [effIndex] / 10);
    Position pos;
    if (m_spellInfo->Effect [effIndex] == SPELL_EFFECT_PULL_TOWARDS_DEST)
    {
        if (m_targets.HasDst()) pos.Relocate(m_targets.m_dstPos);
        else return;
    }
    ...
    unitTarget->GetMotionMaster()->MoveJump(pos.GetPositionX(),
            pos.GetPositionY(), pos.GetPositionZ(), speedXY, speedZ);
}
...
void Spell::EffectDispelMechanic(SpellEffIndex effIndex)
{
    ...
    {
        Aura * aura = itr->second;
        if (!aura->GetApplicationOfTarget(unitTarget->GetGUID())) continue;
        bool success = false;
        GetDispelChance(aura->GetCaster(), unitTarget, aura->GetId(),
                !unitTarget->IsFriendlyTo(m_caster), &success);
        if ((GetAllSpellMechanicMask(aura->GetSpellProto()) & (1 << mechanic))
                && success) dispel_list.push(
                std::make_pair(aura->GetId(), aura->GetCasterGUID()));
    }

    for (; dispel_list.size(); dispel_list.pop())
    {
        unitTarget->RemoveAura(dispel_list.front().first,
                dispel_list.front().second, 0, AURA_REMOVE_BY_ENEMY_SPELL);
    }
}
...
void Spell::EffectSummonDeadPet(SpellEffIndex /*effIndex*/)
{
    ...
    _player->GetMap()->CreatureRelocation(pet, x, y, z,
...
void Spell::EffectDestroyAllTotems(SpellEffIndex /*effIndex*/)
{
    ...
    {
        if (!m_caster->m_SummonSlot [slot]) continue;

        Creature* totem = m_caster->GetMap()->GetCreature(
                m_caster->m_SummonSlot [slot]);
        if (totem && totem->isTotem())
        {
            ...
            if (spellInfo)
            {
                ...
                mana += spellInfo->ManaCostPercentage
                        * m_caster->GetCreateMana() / 100;
            }
    ...
    if (mana) m_caster->CastCustomSpell(m_caster, 39104, &mana, NULL, NULL,
            true);
}
...
void Spell::EffectDurabilityDamage(SpellEffIndex effIndex)
{
    ...
    if (Item* item = unitTarget->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, slot)) unitTarget->ToPlayer()->DurabilityPointsLoss(
            item, damage);
...
void Spell::EffectDurabilityDamagePCT(SpellEffIndex effIndex)
{
    ...
    if (slot < 0)
    {
        unitTarget->ToPlayer()->DurabilityLossAll(double(damage) / 100.0f,
                (slot < -1));
        return;
    }
    ...
    if (Item* item = unitTarget->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, slot)) unitTarget->ToPlayer()->DurabilityLoss(
            item, double(damage) / 100.0f);
}
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    {
        sLog->outErrorDb(
                "Gameobject (Entry: %u) not exist and not created at spell (ID: %u) cast",
    ...
    if (m_targets.HasDst()) m_targets.m_dstPos.GetPosition(fx, fy, fz);
    //FIXME: this can be better check for most objects but still hack
    else if (m_spellInfo->EffectRadiusIndex [effIndex]
            && m_spellInfo->speed == 0)
    {
    ...
    else
    {
        ...
        float min_dis = GetSpellMinRangeForFriend(
                sSpellRangeStore.LookupEntry(m_spellInfo->rangeIndex));
        float max_dis = GetSpellMaxRangeForFriend(
                sSpellRangeStore.LookupEntry(m_spellInfo->rangeIndex));
...
void Spell::EffectProspecting(SpellEffIndex /*effIndex*/)
{
    ...
    Player* p_caster = (Player*) m_caster;
    if (!itemTarget
            || !(itemTarget->GetProto()->Flags & ITEM_PROTO_FLAG_PROSPECTABLE)) return;

    if (itemTarget->GetCount() < 5) return;

    if (sWorld->getBoolConfig(CONFIG_SKILL_PROSPECTING))
    {
        uint32 SkillValue = p_caster->GetPureSkillValue(SKILL_JEWELCRAFTING);
        uint32 reqSkillValue = itemTarget->GetProto()->RequiredSkillRank;
        p_caster->UpdateGatherSkill(SKILL_JEWELCRAFTING, SkillValue,
                reqSkillValue);
    }
...
void Spell::EffectMilling(SpellEffIndex /*effIndex*/)
{
    ...
    Player* p_caster = (Player*) m_caster;
    if (!itemTarget
            || !(itemTarget->GetProto()->Flags & ITEM_PROTO_FLAG_MILLABLE)) return;
    ...
    if (sWorld->getBoolConfig(CONFIG_SKILL_MILLING))
    {
        ...
        uint32 reqSkillValue = itemTarget->GetProto()->RequiredSkillRank;
        p_caster->UpdateGatherSkill(SKILL_INSCRIPTION, SkillValue,
                reqSkillValue);
    }
...
void Spell::EffectStealBeneficialBuff(SpellEffIndex effIndex)
{
            ...
            ++itr)
    {
        ...
        if ((1 << aura->GetSpellProto()->Dispel) & dispelMask)
        {
            ...
            uint8 charges =
                    dispel_charges ?
                            aura->GetCharges() : aura->GetStackAmount();
            if (charges > 0) steal_list.push_back(
                    std::make_pair(aura, charges));
        }
    }

    if (steal_list.empty()) return;
            ...
            if (success)
            {
                success_list.push_back(
                        std::make_pair(itr->first->GetId(),
                                itr->first->GetCasterGUID()));
                --itr->second;
                if (itr->second <= 0) steal_list.erase(itr);
            }
            else
            {
                if (!failCount)
            ...
            itr != success_list.end(); ++itr)
    {
        ...
        unitTarget->RemoveAurasDueToSpellBySteal(itr->first, itr->second,
                m_caster);
    }
    m_caster->SendMessageToSet(&dataSuccess, true);
}
...
void Spell::EffectKillCreditPersonal(SpellEffIndex effIndex)
{
    ...
    unitTarget->ToPlayer()->KilledMonsterCredit(
            m_spellInfo->EffectMiscValue [effIndex], 0);
}
...
void Spell::EffectKillCredit(SpellEffIndex effIndex)
{
    ...
    if (creatureEntry) unitTarget->ToPlayer()->RewardPlayerAndGroupAtEvent(
            creatureEntry, unitTarget);
}
...
void Spell::EffectQuestStart(SpellEffIndex effIndex)
{
    ...
    if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(m_spellInfo->EffectMiscValue[effIndex]))
    {
        if (player->CanTakeQuest(qInfo, false)
                && player->CanAddQuest(qInfo, false))
        {
...
void Spell::EffectActivateRune(SpellEffIndex effIndex)
{
    ...
    if (plr->getClass() != CLASS_DEATH_KNIGHT) return;
...
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER
            || unitTarget->GetPetGUID()
            || unitTarget->getClass() != CLASS_HUNTER) return;
    ...
    unitTarget->SetMinion(pet, true,
            unitTarget->ToPlayer()->getSlotForNewPet());
...
void Spell::EffectDiscoverTaxi(SpellEffIndex effIndex)
{
    ...
    if (sTaxiNodesStore.LookupEntry(nodeid)) unitTarget->ToPlayer()->GetSession()->SendDiscoverNewTaxiNode(
            nodeid);
}

void Spell::EffectTitanGrip(SpellEffIndex /*effIndex*/)
{
    if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER) unitTarget->ToPlayer()->SetCanTitanGrip(
            true);
}

void Spell::EffectRedirectThreat(SpellEffIndex /*effIndex*/)
{
    if (unitTarget) m_caster->SetReducedThreatPercent((uint32) damage,
            unitTarget->GetGUID());
}
...
void Spell::EffectWMODamage(SpellEffIndex /*effIndex*/)
{
                    ...
                    == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        if (gameObjTarget->GetGOValue()->building.health == 0) return;

        FactionTemplateEntry const *casterft, *goft;
        casterft = caster->getFactionTemplateEntry();
        goft = sFactionTemplateStore.LookupEntry(
                gameObjTarget->GetUInt32Value(GAMEOBJECT_FACTION));
        ...
        if ((casterft && goft && !casterft->IsFriendlyTo(*goft)) || !goft)
        {
            ...
            if (Unit *who = caster->GetCharmerOrOwner()) data.append(
                    who->GetPackGUID());
            else data << uint8(0);
            data << uint32(damage);
            data << uint32(m_spellInfo->Id);
...
void Spell::EffectWMORepair(SpellEffIndex /*effIndex*/)
{
    if (gameObjTarget
            && gameObjTarget->GetGoType()
                    == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING) gameObjTarget->Rebuild();
}
...
void Spell::EffectWMOChange(SpellEffIndex effIndex)
{
                    ...
                    == GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)
    {
        ...
        {
            case 0: // intact
                if (gameObjTarget->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_DAMAGED)) gameObjTarget->RemoveFlag(
                        GAMEOBJECT_FLAGS, GO_FLAG_DAMAGED);
                if (gameObjTarget->HasFlag(GAMEOBJECT_FLAGS, GO_FLAG_DESTROYED)) gameObjTarget->RemoveFlag(
        ...
        uint32 count)
{
    ...
    if (m_targets.HasDst())
    {
        ...
        if (count == 0) pos.Relocate(m_targets.m_dstPos);
        // Summon in random point all other units if location present
        else
        {
...
{
    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT
            || !unitTarget->ToCreature()->isPet()
            || ((Pet*) unitTarget)->getPetType() != HUNTER_PET) return;
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex)
{
    ...
    {
        sLog->outError("EffectPlayMusic: Sound (Id: %u) not exist in spell %u.",
...
void Spell::EffectPlayerNotification(SpellEffIndex /*effIndex*/)
{
    ...
    OutdoorPvPWG *pvpWG = (OutdoorPvPWG*) sOutdoorPvPMgr->GetOutdoorPvPToZoneId(
            4197);
    ...
    switch (m_spellInfo->Id)
    {
        case 58730: // Restricted Flight Area
        {
            if (sWorld->getBoolConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED))
            {
                ...
                {
                    unitTarget->ToPlayer()->GetSession()->SendNotification(
                            LANG_ZONE_NOFLYZONE);
                    unitTarget->PlayDirectSound(9417); // Fel Reaver sound
                    unitTarget->MonsterTextEmote(
                            "The air is too thin in Wintergrasp for normal flight. You will be ejected in 9 sec.",
                            unitTarget->GetGUID(), true);
                    break;
                }
...
void Spell::EffectBind(SpellEffIndex effIndex)
{
    ...
    uint32 area_id;
    WorldLocation loc;
    if (m_spellInfo->EffectImplicitTargetA [effIndex] == TARGET_DST_DB
            || m_spellInfo->EffectImplicitTargetB [effIndex] == TARGET_DST_DB)
    {
        SpellTargetPosition const* st = sSpellMgr->GetSpellTargetPosition(
                m_spellInfo->Id);
        if (!st)
        {
            sLog->outError(
                    "Spell::EffectBind - unknown teleport coordinates for spell ID %u",
                    m_spellInfo->Id);
            return;
        }
...
void Spell::EffectDamageSelfPct(SpellEffIndex effIndex)
{
    ...
    m_damage += m_originalCaster->SpellDamageBonus(unitTarget, m_spellInfo,
            effIndex, unitTarget->CountPctFromMaxHealth(damage), SELF_DAMAGE);
}
